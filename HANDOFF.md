...
Latest Phase 1 batch: `UD/FD/ND` project media material construction now starts from the source defaults. Source `UD` constructs `tMap:null`, zero `uContainerSize`/`uMapSize`, zero `uBackgroundColor`, `uReveal:0`, `toneMapped:false`, `transparent:true`, `depthWrite:false`, and `depthTest:false`; source `ND` and `FD` own later map-size, texture, container-size, radius, and background writes. The rebuild removed the constructor-time placeholder texture, `1x1`/`1600x1200` size fallbacks, and project-background clone, added runtime probe coverage for every project media plane, and extended project-media/audit tooling to reject restoring those non-source defaults. This is one project-media material lifecycle edge only. Phase 1 is still open.
