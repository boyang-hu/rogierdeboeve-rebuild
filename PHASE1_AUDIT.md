...
This table is the immediate execution board for the next implementation batches. It replaces the older habit of committing one tiny cleanup at a time. Each row should be handled as one focused batch or experiment, with build/marker/browser QA before commit.

| Priority | ID | Chain | Source evidence | Rebuild state | Proposed batch | Risk |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | S1-21 | Full ordinary `VA` vertex shader parity | Source `VA.onBeforeCompile` assigns complete `HA` and `zA`. `HA` owns perlin displacement, reveal fade, spread, mouse scaling, displacement-wave z offset, instance spread, `vViewPosition`, and world-position output. | Rebuild injects chunks into current Three 0.184 shaders and covers the broad intent, but it does not yet guarantee the same vertex program or light-facing varyings. | Create an isolated full-`HA` ordinary work-block experiment behind a local material path. Compare shader compile/runtime/luma before touching auxiliary `WA/XA`. Keep or revert based on objective QA. | High |
| 2 | S1-22 | Full ordinary `VA` fragment light body | Source `zA` includes only selected standard chunks, computes `totalDiffuse + totalSpecular + totalEmissiveRadiance`, then applies the custom alpha/mouse tail. The source bundle's Three light chunks may differ materially from Three 0.184's generated `MeshStandardMaterial` chunks. | Rebuild strips many source-omitted chunks but still depends on Three 0.184 standard-material structure for the light body and `SpotLight.map` plumbing. | If S1-21 compiles, pair it with a full source `zA` experiment or a generated-shader diff tool that shows exactly which light structs/functions differ. Do not mix with composite changes. | High |
| 3 | S1-23 | Spotlight-map contribution attribution | Source `SD.init()` assigns `workScene.spotLight.map = workThumbScene.renderManager.renderTargetComposite.texture`; `p1.update()` only parallax-adjusts spotlight `x/y`. Source `VA` relies on standard light chunks from its bundled Three version for projection. | Rebuild ownership, intensity, position, target, angle, penumbra, and parallax are source-shaped. The likely gap is shader/light-version semantics, not a missing map assignment. | Add a debug-only measurement/capture pass or temporary branch that isolates blocks with and without `SpotLight.map`. Keep no tuning constants unless source-proven. | Medium-high |
