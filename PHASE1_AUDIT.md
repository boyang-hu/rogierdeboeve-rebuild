...
- Ordinary `VA-work` now uses direct source-shaped `HA/zA` templates and the generated residual report shows vertex/fragment deltas `0`. The raw `uUvOffset` shader declaration is now source-aligned as `vec3`; the remaining documented bridge is the mirrored runtime shape where source `VA.customUniforms` constructs `uUvOffset` from `Vector2`, source `GA` writes only `.x/.y`, and the source shader reads `uUvOffset.xy`. The old source `SPECULAR` macro is restored in `zA`; runtime probes guard that ordinary work is `MeshStandardMaterial`, not `MeshPhysicalMaterial`, so the `PHYSICAL` branch is inactive. Auxiliary block materials now use source `XA/KA` constructor state and direct source `jA/WA/YA/qA` shader surfaces, guarded separately from ordinary `VA-work`.
- `A1/OA/kA/Lu/I1` are source-shaped in pieces and now have source-shaped `A1/OA/lA` composite shader fragments, source-shaped `u1/z1` sky/environment shader fragments, source-shaped `nD/Iu/Lu/I1/Lo` update sequencing, `Lo/Lu/I1` render-target clone ownership, source-shaped `I1` optional blur target chaining, `a1/i1` floor-reflection uniform ownership, pass-material ownership, and source `Lu/kA/I1` render-manager settings ownership, but transfer interpretation and composite behavior across the whole chain are still not byte-for-byte source-isomorphic.
...
- Source `VA` constructs `uReveal:new I(0)`, `uMouseLightness:new I(1)`, `tMouseSim:new I(null)`, `tMouseSim2:new I(null)`, and `tDisplacement:new I(null)`.
- Source `XA` and `KA` construct the same sampler and mouse-lightness defaults; their reveal defaults stay source-specific (`XA uReveal=0/uRevealSpread=1`, `KA uReveal=0/uRevealSpread=10`).
- Source `yD.animateIn()` and `yD.onProjectActive()` own runtime `uReveal` writes, and source `Se.setThumbMouseLightness()` owns runtime `uMouseLightness` fan-out.
- Source `GA.update()` owns ordinary work runtime `tMouseSim`, `uMouseSpeed`, and `tDisplacement` writes; source `p1.update()` owns ordinary work `tMouseSim2`.
- Source `$A.update()` owns about runtime local `Ka` writeback for `tMouseSim`, `uMouseSpeed`, and `tDisplacement`; source `p1.update()` then writes about `tMouseSim2`.
- Source `ZA.update()` only updates material time and floating positions, so floating `KA` sampler uniforms remain constructor-null unless source evidence proves another writer.

Runtime and tooling changes:

- Ordinary work material construction now uses source defaults for `uReveal`, `uMouseLightness`, `tMouseSim`, `tMouseSim2`, and `tDisplacement`.
- About and floating auxiliary material construction now uses source-null sampler defaults and records constructor evidence alongside the existing shader-mode evidence.
- About blocks now get their own source-shaped local mouse simulation state, source ray-plane scale/position, target sizing, pointer projection, and `$A.update()`-style runtime writeback for `tMouseSim`, `uMouseSpeed`, and `tDisplacement`.
- Floating blocks no longer receive rebuild-owned `tMouseSim2` or `tDisplacement` writes from the shared auxiliary update path.
- `__rogierOutputProbe.settings.work.activeMaterial`, `.auxiliaryMaterial`, and `.floatingAuxiliaryMaterial` expose constructor-default evidence and runtime binding ownership.
- `scripts/probe-output-color.mjs` hard-fails on constructor-default drift and floating sampler runtime drift; about runtime binding only fails if a visible about route reports a false runtime check.
- `scripts/audit-renderer-output.mjs` checks source `VA/XA/KA`, `GA`, `$A`, `ZA`, `p1`, and `Se` anchors plus rebuild/probe coverage.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-constructor-defaults-audit.json` passed.
- Recursive false/null extraction from `/tmp/rd-constructor-defaults-audit.json` printed `false/null entries 0`.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed: `/tmp/rd-constructor-defaults-output-desktop`.
- Mobile output probe passed: `/tmp/rd-constructor-defaults-output-mobile`.
- Thumb spotlight probe passed: `/tmp/rd-constructor-defaults-thumb`.
- Project-media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining `5/5` visible media tracks on both pages: `/tmp/rd-constructor-defaults-media`.
- Interactive mouse probe passed: `/tmp/rd-constructor-defaults-interactive`.
- About/full route smoke via `scripts/capture.mjs` passed with no failures or exceptions for `rebuild-about-desktop`: `/tmp/rd-constructor-defaults-about-smoke`.

Decision: keep `VA/XA/KA` constructor defaults source-owned, leave runtime writes to the source owner paths (`yD`, `Se`, `GA`, `p1`, `$A`), and keep `ZA/KA` floating sampler uniforms constructor-null until mirrored-bundle evidence identifies a writer. Phase 1 remains open because this closes constructor/runtime ownership in the block material chain only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-313 `VA/XA/KA` `uMouseSpeed` Constructor Ownership

This batch aligns a narrow work/auxiliary block material constructor default. It does not change shader text, render targets, project data, route behavior, visual constants, or the source `GA.update()` runtime speed write.

Source evidence:

- Source `VA` constructs `customUniforms.uMouseSpeed` with `new I(null)`.
- Source `XA` and `KA` also construct `customUniforms.uMouseSpeed` with `new I(null)`.
- Source `GA.update()` later writes `this.material.customUniforms.uMouseSpeed.value=this.mouseSpeed`, so the constructor-null default and the runtime numeric value have separate ownership.

Runtime and tooling changes:

- `src/client/webgl.ts` now constructs ordinary work, about auxiliary, and floating auxiliary block materials with `uMouseSpeed: { value: null }`.
- Each material records `source-VA-XA-KA-uMouseSpeed-construct-null-GA-update-writes-runtime` and a constructor-null boolean in `userData`.
- `__rogierOutputProbe.settings.work.activeMaterial`, `.auxiliaryMaterial`, and `.floatingAuxiliaryMaterial` expose those constructor markers while existing `p1.update` checks keep guarding the runtime `GA.update()` write.
- `scripts/probe-output-color.mjs` now fails if any of the three material classes lose the constructor-null evidence.
- `scripts/audit-renderer-output.mjs` now checks source `VA/XA/KA` constructor-null anchors, source `GA.update()` runtime write, rebuild constructor/runtime anchors, and probe coverage.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-umousespeed-null-audit.json` passed; `sourceManagers.GA.uMouseSpeedConstructorOwnership` reported all true values.
- Recursive false/null extraction from `/tmp/rd-umousespeed-null-audit.json` printed no entries.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed: `/tmp/rd-umousespeed-null-output-desktop`.
- Mobile output probe passed: `/tmp/rd-umousespeed-null-output-mobile`.
- Thumb spotlight probe passed: `/tmp/rd-umousespeed-null-thumb`.
- Project-media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining `5/5` visible media tracks on both pages: `/tmp/rd-umousespeed-null-media`.

Decision: keep `uMouseSpeed` constructor-null for source `VA/XA/KA` and leave numeric speed ownership to the source `GA.update()` runtime write. Phase 1 remains open because this closes one constructor/runtime ownership edge only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-312 Renderer Target Diagnostic Normalization

This batch cleans the renderer audit surface without changing production rendering, shader text, render targets, route behavior, project data, or visual constants.

Source and local evidence:

- Source `Lo` constructs depthless/stencilless targets with `new Dn(1,1,{depthBuffer:!1,stencilBuffer:!1})`, then clones them for composite-style managers.
- Source `Lu` and main `I1` already have separate guarded target-state ownership for their raw/composite/bloom/blur/fxaa targets.
- Local Three r164 `WebGLRenderTarget` defaults include `texture.generateMipmaps=false`; the source-style depthless target also has `depthBuffer=false` and `stencilBuffer=false`.
- The Node audit environment may not expose `OffscreenCanvas`, so a renderer-default probe can be unavailable there even though browser probes cover the runtime renderer.

Tooling changes:

- `scripts/audit-renderer-output.mjs` now wraps render-target boolean defaults as `actual`, `expected`, and `matchesExpected` diagnostics.
- Source-like depthless targets explicitly expect `generateMipmaps=false`, `depthBuffer=false`, and `stencilBuffer=false`.
- Default local Three targets explicitly expect `generateMipmaps=false`, `depthBuffer=true`, and `stencilBuffer=false`.
- The renderer-default probe now reports `status: "unavailable"` with a reason when `OffscreenCanvas` is absent instead of returning `null`.

Verification:

- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-audit-diagnostics-cleanup.json` passed.
- Recursive false/null extraction from `/tmp/rd-audit-diagnostics-cleanup.json` printed no entries.

Decision: keep expected false render-target defaults visible as structured diagnostics so future Phase 1 audits only surface real failing checks. This is QA noise removal only; Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.

### S1-311 `yD/Qe.workState` Gallery Scroll Persistence Ownership

This batch extends the source-backed Home gallery state persistence that feeds gallery progress, active project state, and route-return projection continuity. It does not change shader text, render targets, route media behavior, project data, or visual constants.

Source evidence:

- Source `yD.init()` creates `this.scroll` with `virtual`, `target`, `animated`, `current`, `progress`, `limit`, `velocity`, `diff`, `remainder`, `step`, `offset`, and `active`.
- Source restores state with `Qe.workState&&(this.scroll=Qe.workState.scroll,this.index=Qe.workState.index,this.activeProject=Qe.workState.activeProject,this.activeHook=Qe.workState.activeHook,this.targetHook=Qe.workState.targetHook,this.sceneRotation=Qe.workState.sceneRotation)`.
- Source `yD.onRaf()` adds `this.scroll.targetPlusDiff=this.scroll.target+this.scroll.diff` before damping `scroll.animated`.
- Source `yD.destroy()` saves the full scroll object by reference in `Qe.workState={scroll:this.scroll,index:this.index,activeProject:this.activeProject,activeHook:this.activeHook,targetHook:this.targetHook,sceneRotation:this.sceneRotation}`.

Runtime and tooling changes:

- `src/client/main.ts` now types the gallery scroll state with source runtime fields, including optional `targetPlusDiff`.
- The RAF path now writes `scroll.targetPlusDiff` as a source state field before using it for `scroll.animated`.
- Session-backed `rd:work-state` now persists `diff`, `targetPlusDiff`, `velocity`, and `active` alongside the existing source fields. The existing restore gate still sets `scroll.active=false` until the Home gallery entry lifecycle reactivates WebGL, preserving the earlier source-entry timing guardrail.
- `scripts/audit-renderer-output.mjs` now checks source `Qe.workState` save/restore anchors plus rebuild persistence of the source scroll fields.
...
- `node scripts/audit-renderer-output.mjs > /tmp/rd-workstate-audit-final.json` passed; `sourceManagers.homeGalleryUpdateScene.scrollRuntime` and `.workStatePersistence` both reported `{ "source": true, "rebuild": true }`.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
...
Decision: keep Home gallery state persistence aligned with source `Qe.workState` for scroll runtime fields instead of saving only a partial scroll snapshot. Phase 1 remains open because this closes route-state continuity only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-310 `yD` Gallery Scroll Runtime Rounding Ownership

This batch aligns the source `yD.onRaf()` scroll runtime that feeds `yD.updateScene()` and the spotlight/thumb projection chain. It does not change shader text, render targets, project data, route media behavior, or visual constants.

Source evidence:

- Source `yD.onRaf({time:e,delta:t,frame:n,fps:i})` computes `this.scroll.velocity=this.scroll.target-this.scroll.animated`.
- Source calls `this.checkSpeed()` and, when snapping, damps `this.scroll.diff` through `Yi(this.scroll.diff,0,5,t)`.
- Source sets `this.scroll.targetPlusDiff=this.scroll.target+this.scroll.diff`.
- Source damps `this.scroll.animated` through `Yi(this.scroll.animated,this.scroll.targetPlusDiff,5,t)`, then derives `current`, `progress`, active item state, and `updateScene(t)`.
- Source `yD.updateScene(e)` computes the roll target with `bo(this.scroll.velocity*-.015,-4,4)` and persists `this.sceneRotation=Yi(this.sceneRotation,n,5,e)`.
- Source `bo()` clamps and rounds through `Fn(...,4)`, and source `Yi()` uses source-rounded exponential lerp through `PT/Fn`.

Runtime and tooling changes:

- `src/client/main.ts` now replaces the local unrounded `lerp()` helper in the Home work gallery controller with `sourceRound()`, `sourceDamp()`, and `sourceClampRound()`.
- `tick()` now damps `scroll.diff` and `scroll.animated` through `sourceDamp()`, preserving source four-decimal rounding on the scroll values that feed gallery progress and active project state.
- The route-level persisted `sceneRotation` now uses a source-clamped rounded roll target plus source-rounded damp, matching the `bo/Yi` ownership already used inside the WebGL gallery dynamics path.
- `scripts/audit-renderer-output.mjs` now extracts `class yD extends Ht` before checking `onRaf` anchors, avoiding unrelated global `onRaf` matches.
- Renderer audit records `sourceManagers.homeGalleryUpdateScene.scrollRuntime.source=true` and `.rebuild=true`, and rejects restoring the old unrounded local `lerp(...)` calls for `scroll.diff`, `scroll.animated`, or `sceneRotation`.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-gallery-scroll-audit-final.json` passed; `sourceManagers.homeGalleryUpdateScene.scrollRuntime` reported `{ "source": true, "rebuild": true }`.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `CHROME_PATH=/usr/bin/google-chrome-stable REBUILD_URL=http://localhost:5180`.
- Desktop thumb spotlight probe passed.
- Project-media probe passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks.

Decision: keep gallery scroll `diff`, `animated`, and route-level `sceneRotation` on source-rounded `Yi/bo` ownership. Do not reintroduce unrounded local interpolation in the Home work gallery runtime without mirrored-bundle evidence. Phase 1 remains open because this closes scroll runtime ownership only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-309 `Lu/GA/Ka` Mouse Simulation Resize Ownership

This batch aligns an active `Ka` mouse-simulation resize ownership edge. It does not change shader text, pass order, route data, project media, visual constants, or the normal desktop/mobile target dimensions.

Source evidence:

- Source `Lu.resize()` rounds render size, then calls `this.mouseSimulation.onResize(e/10,t/10)` without a clamp or post-rounding.
- Source `GA.resize(e,t,n)` calls `this.mouseSim.onResize(this.plane.scale.x,this.plane.scale.y)` without a clamp or post-rounding.
- Source `Ka.onResize(e,t)` directly calls `this.bufferSim.onResize(e,t)` and writes `this.simulationMaterial.uniforms.uCoords.value.set(e,t)`.

Runtime and tooling changes:

- Screen mouse simulation now sizes targets and `uCoords` from `workRenderWidth / 10` and `workRenderHeight / 10` directly, removing the rebuild-only `Math.max(1, ...)` clamp.
- Per-work-item local mouse simulation now sizes targets and `uCoords` from `item.mousePlane.scale.x/y` directly, removing the rebuild-only `Math.max(1, ...)` clamp.
- `__rogierOutputProbe.mouseSimulation` exposes source no-clamp sizing modes for screen and active local mouse simulation.
- `scripts/probe-output-color.mjs` and `scripts/probe-interactive-mouse.mjs` now hard-fail if screen or local mouse simulation sizing mode drifts from source no-clamp ownership.
- `scripts/audit-renderer-output.mjs` now checks the `Lu`, `GA`, and `Ka` source anchors and rejects restoring the old clamp or rounding around these resize inputs.
...
- `node scripts/audit-renderer-output.mjs > /tmp/rd-mousesim-resize-audit-final.json` passed; `sourceManagers.Lu.screenOwnership.sourceMouseSimResizeNoPostRound`, `sourceManagers.Lu.screenOwnership.rebuildScreenMouseSimResizeNoPostRound`, and `sourceManagers.GA.localMouseSimResizeOwnership` were true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `CHROME_PATH=/usr/bin/google-chrome-stable REBUILD_URL=http://localhost:5180`.
- Desktop thumb spotlight probe passed.
- Project-media probe passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks.
- Interactive mouse probe passed and asserted screen/local mouse-simulation no-clamp sizing modes plus source-shaped response.

Decision: keep active screen and local `Ka` mouse-simulation resize inputs source-shaped with no rebuild clamp or post-rounding. Phase 1 remains open because this closes resize ownership only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-308 `Lu/I1 rg` Bloom Direction Runtime Ownership

This batch aligns a runtime uniform-ownership edge in the source `Lu/I1` bloom chain. It does not change shader text, pass order, target sizing formulas, route data, project media, or visual constants.

Source evidence:

- Source `Lu` assigns `this.blurMaterials[p].uniforms.uDirection.value=bf` for the horizontal bloom pass and `this.blurMaterials[p].uniforms.uDirection.value=Mf` for the vertical pass.
- Source `I1` assigns `this.blurMaterials[p].uniforms.uDirection.value=wf` for the horizontal bloom pass and `this.blurMaterials[p].uniforms.uDirection.value=Tf` for the vertical pass.
- Source `rg` still constructs `uDirection` as `new Q(.5,.5)`; the enabled bloom render loop replaces the uniform value object with the source shared direction vectors, while a disabled bloom branch leaves the constructor vector untouched.
- Source `Na` receives its horizontal/vertical direction vectors through construction, so it does not need a redundant post-constructor `.set(...)` write.

Runtime and tooling changes:

- `WebGLBackdrop` now owns separate source work/main horizontal and vertical bloom direction vectors.
- `renderBloomChain()` now receives those vectors and assigns `blurMaterial.uniforms.uDirection.value = horizontalDirection` / `verticalDirection`, matching source `Lu/I1` value-object replacement rather than mutating each constructor vector.
- Removed redundant post-constructor `.set(1,0)` / `.set(0,1)` writes from standard blur material setup; `Na` direction stays constructor-owned.
- `__rogierOutputProbe.uniforms.passMaterials` exposes the source direction assignment mode, runtime direction vectors, object identity against the source shared vectors, disabled-bloom constructor retention, and `Na` constructor direction ownership.
- `scripts/probe-output-color.mjs` now hard-fails if enabled bloom does not leave every `rg` material on shared vertical `[0,1]`, if disabled bloom does not keep `[0.5,0.5]`, or if standard blur constructor direction ownership drifts.
- `scripts/audit-renderer-output.mjs` now checks the source `Lu/I1` direction assignment anchors and the rebuild shared-vector assignment/no-`.set()` anchors.

Verification:

- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-bloom-direction-audit.json` passed; `sourceBloomDirectionRuntimeAssignment`, `rebuildBloomDirectionRuntimeAssignment`, and `rebuildStandardBlurConstructorOwnsDirection` were true.
- `git diff --check` passed.
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-bloom-direction-audit-final.json` passed; `LuHorizontal`, `LuVertical`, `I1Horizontal`, `I1Vertical`, `rebuildBloomDirectionRuntimeAssignment`, and `rebuildStandardBlurConstructorOwnsDirection` were true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `CHROME_PATH=/usr/bin/google-chrome-stable REBUILD_URL=http://localhost:5180`.
- Desktop thumb spotlight probe passed.
- Project-media probe passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks.

Decision: keep bloom blur `rg.uDirection` runtime ownership source-shaped as shared direction-vector assignment, and keep standard blur `Na.uDirection` constructor-owned. Phase 1 remains open because this closes runtime uniform ownership only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.
...
- Source `XA.onBeforeCompile` assigns `t.vertexShader=jA` and `t.fragmentShader=WA`.
- Source `KA.onBeforeCompile` assigns `t.vertexShader=YA` and `t.fragmentShader=qA`.
- Source `jA` uses about-specific reveal displacement: `float revealCombined = uReveal`, the `10.25` perlin mix, and `waveDisplacement = ... 9. * (1. - revealCombined)`.
- Source `WA` keeps the standard fragment include surface, sets `revealCombined = 1.`, uses mouse alpha `uMouseFactor * 0.15`, and writes `gl_FragColor.a = mixedAlpha * uScrollOpacity`.
- Source `YA` uses floating-specific perlin height `5.`, samples `tPerlin` without the time offset, and applies the source `transformed.z *= 7. * (.5 + instanceColor.r)` path.
- Source `qA` clamps alpha with `clamp(mixedAlpha, 0.05, 1.)` and writes `gl_FragColor.a = mixedAlpha * uReveal`.

Runtime and tooling changes:

- `createAuxiliaryBlockMaterial("about")` now routes `onBeforeCompile` to direct source `jA/WA`.
- `createAuxiliaryBlockMaterial("floating")` now routes `onBeforeCompile` to direct source `YA/qA`.
- Removed the rebuild-only auxiliary opaque chunk and `uAuxiliaryMaterial` production bridge from the auxiliary material path.
- `__rogierOutputProbe.settings.work.auxiliaryMaterial` and `floatingAuxiliaryMaterial` now expose `source-XA-jA-WA-direct-shader` and `source-KA-YA-qA-direct-shader`.
- `scripts/audit-renderer-output.mjs` now extracts `jA/WA/YA/qA`, checks source/rebuild/probe/dump anchors, and rejects the old auxiliary chunk bridge.
- `scripts/dump-va-shader.mjs` can now compare `XA-about` and `KA-floating` source templates when those shader programs are dumped, and it no longer fails on about-only dumps that do not compile ordinary `VA-work`.
- `scripts/probe-output-color.mjs` hard-fails if the auxiliary material shader modes drift.
...
- `node scripts/audit-renderer-output.mjs > /tmp/rd-aux-shader-audit.json` passed; `sourceManagers.auxiliaryBlockShaders` source, rebuild, no-old-bridge, dump, and probe checks were true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Static `/about/` CDP smoke passed with body `is-about is-ready has-entered has-webgl`, full-size canvas, no failed requests, no exceptions, and no shader/WebGL console messages.
- Home shader dump passed at `/tmp/rd-aux-home-shader-dump`; ordinary `VA-work` remained source-shaped with vertex/fragment delta `0`.
- Desktop output probe passed and asserted the new auxiliary shader modes.
- Desktop thumb spotlight probe passed.
- Project-media probe passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks.

Decision: keep auxiliary material shader ownership on source `XA/KA` direct `jA/WA/YA/qA` surfaces. Phase 1 remains open because this closes the auxiliary shader bridge only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-305 `Q1/eD/TD` Character Rotatable Lifecycle

This batch aligns the source about character rotatable lifecycle. It does not change shader formulas, visual constants, route data, project media, or start Phase 2.

Source evidence:

- Source `TD.addEvents()` waits `100ms`, registers `Bt.add(this.onRaf,"aboutVisual")`, assigns `J.workScene.spotLight.map=J.characterScene.renderManager.renderTargetComposite.texture`, then calls `J.characterScene.character.rotatableMesh.addEvents()` before force resize.
- Source `TD.destroy()` calls `J.characterScene.character.rotatableMesh.removeEvents()`.
- Source `Q1` defaults to `{ horizontal:true, vertical:false, dampingFactor:5 }`, registers passive mouse/touch start/move/end events on `window`, and removes the same handlers.
- Source `Q1.onMouseMove()` writes target rotation from pointer movement at `*.01`; source `Q1.update(delta)` damps horizontal rotation through `Yi(..., dampingFactor, delta)`.
- Source `eD` builds `cameraPanGroup -> rotatableMesh -> character`, creates `new Q1({horizontal:true, vertical:false})`, lerps mouse toward `Pe.mouse.normalized` with `.1`, clamps camera pan x to `[-.15,.3]`, and auto-rotates the character with `rotation.y += delta * 1`.

Runtime and tooling changes:

- The rebuild character scene now mirrors the source wrapper hierarchy: `characterCameraPanGroup -> characterRotatableMesh -> characterBodyGroup -> fallback/model`.
- `scheduleAboutVisualLifecycle()` now calls `sourceAddCharacterRotatableEvents()` immediately after the delayed character spotlight map bind, matching the source TD 100ms point.
- About out/destroy and WebGL destroy paths call `sourceRemoveCharacterRotatableEvents()` so the window event handlers do not leak outside about visual ownership.
- The character target render path now updates source-shaped rotatable mesh damping, camera pan lerp/clamp, and character auto-rotation before rendering the character target while the about blocks are visible.
- `__rogierOutputProbe.settings.work.auxiliaryLifecycle` and `__rogierOutputProbe.reflectionState.auxiliary` expose rotatable mode, wrapper mode, event mode, update mode, active-event state, hierarchy counts, rotation, and source constants.
- `scripts/audit-renderer-output.mjs` records `sourceManagers.TDCharacterRotatableLifecycle` and hard-checks `Q1`, `eD`, `TD`, rebuild, and probe anchors.
- `scripts/probe-output-color.mjs` hard-fails on character rotatable lifecycle, wrapper, event, update, axis, damping, auto-rotate, or camera-pan clamp drift.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-character-rotatable-audit.json` passed; `sourceManagers.TDCharacterRotatableLifecycle` source, rebuild, and output-probe checks were true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Static `/about/` CDP drag smoke passed with body `is-about is-ready has-entered has-webgl`, rotatable mode `source-TD-character-rotatableMesh-addEvents-after-map-remove-on-destroy`, events active after the delayed map bind, source wrapper hierarchy present, and dragged rotation changing from `0` to `1.4511`.

Decision: keep source `Q1/eD/TD` character rotatable wrapper, events, update, and cleanup ownership. Phase 1 remains open because this closes about-route character lifecycle timing/interaction only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, floor/environment residuals, and the auxiliary shader bridge remain unresolved.

### S1-304 `TD` About Visual Lifecycle

This batch aligns the source about visual lifecycle timing. It does not change shader formulas, visual constants, route data, project media, or start Phase 2.

Source evidence:

- Source `TD.addEvents()` waits for page scroll and `J.initPromise`, then calls `Se.setSpotLightIntensity(0,0)`.
- Source `TD.addEvents()` assigns `J.workScene.aboutBlocks.track=this.el`, sets `aboutBlocks.visible=true`, and disables `spotLightParallax`.
- Source `TD.addEvents()` waits `100ms`, registers `Bt.add(this.onRaf,"aboutVisual")`, binds `J.workScene.spotLight.map` to `J.characterScene.renderManager.renderTargetComposite.texture`, registers character rotatable events, and emits `pe.emit(xe.FORCE_RESIZE)`.
- Source `TD.addEvents()` waits another `200ms`, then calls `this.onScroll()`.
- Source `TD.destroy()` removes `aboutVisual`, clears the about block track/visible state, restores `spotLightParallax`, and removes character rotatable events.

Runtime and tooling changes:

- `enterAboutVisualState()` no longer binds the character composite spotlight map, forces resize, or applies about scroll state immediately.
- `scheduleAboutVisualLifecycle()` now mirrors the source timing: a `100ms` delay enables the about visual RAF path, binds `spotLight.map` to the character composite texture, and calls `resize()`; a nested `200ms` delay then applies the initial about scroll/spotlight state.
- About scroll opacity and about spotlight updates now run only after the delayed about visual RAF registration point, matching source `Bt.add(this.onRaf,"aboutVisual")`.
- About out/destroy paths clear both lifecycle timers and disable the delayed RAF gate.
- `__rogierOutputProbe.settings.work.auxiliaryLifecycle` and `__rogierOutputProbe.reflectionState.auxiliary` expose the lifecycle mode, delay values, map-binding state, resize marker, RAF marker, and initial-scroll marker.
- `scripts/audit-renderer-output.mjs` records `sourceManagers.TDAboutVisualLifecycle` and hard-checks the source anchors plus rebuild/probe coverage.
- `scripts/probe-output-color.mjs` hard-fails on about visual lifecycle delay, map binding, resize, or initial-scroll mode drift.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-td-about-lifecycle-audit.json` passed; `sourceManagers.TDAboutVisualLifecycle` source, rebuild, and output-probe checks were true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Static `/about/` CDP smoke passed with body `is-about is-ready has-entered has-webgl`, lifecycle mode `source-TD-addEvents-100ms-map-resize-200ms-initial-scroll`, `aboutVisible=true`, `floatingVisible=true`, character composite map binding after delay, forced resize after map binding, and initial scroll after the nested delay.
- Desktop `scripts/probe-output-color.mjs` passed.
- Desktop `scripts/probe-thumb-spotlight.mjs` passed.
- `scripts/probe-project-media.mjs` passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks.

Decision: keep source `TD` about visual timing for map binding, forced resize, RAF activation, and initial scroll. Phase 1 remains open because this closes about-route lifecycle timing only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, floor/environment residuals, and the auxiliary shader bridge remain unresolved.

### S1-303 `Fg` Floating Block Lifecycle and Page Scroll Velocity

This batch aligns the source about floating-block component lifecycle without changing visual constants, route data, auxiliary shader text, or starting Phase 2.

Source evidence:

- Source `Fg.addEvents()` waits for `J.initPromise`, sets `this.translation=.005`, and registers `onRaf`.
- Source `Fg.onRaf(e)` calls `J.workScene.floatingBlocks.update(e)` and then adds `this.translation * Math.abs(this.page.scroll.velocity)` to `floatingBlocks.translationZ`.
- Source `Fg.animateIn()` sets `J.workScene.floatingBlocks.visible=true` in the `uReveal` tween `onStart`, not during addEvents/setup.
- Source `Fg.animateOut()` sets `J.workScene.floatingBlocks.visible=false` in the `uReveal` tween `onComplete`.

Runtime and tooling changes:

- `enterAboutVisualState()` now keeps floating blocks hidden after setup; `animateAboutVisualIn()` owns the visible flip through the floating `uReveal` tween `onStart`.
- `src/client/motion.ts` now broadcasts Lenis `scroll` and `velocity` through `rd:page-scroll`; the about route forwards that state to WebGL through `setAboutScrollState()`.
- `updateAuxiliaryBlocks()` now consumes the page-scroll velocity path for the source `.005 * abs(velocity)` `translationZ` feed, with a window-scroll delta fallback only when Lenis/reduced-motion events are unavailable.
- `__rogierOutputProbe` exposes `auxiliaryLifecycle`, `floatingVisibilityMode`, `floatingScrollVelocityMode`, `floatingTranslation`, and the active scroll-velocity source.
- `scripts/audit-renderer-output.mjs` extracts source `Fg` and hard-checks the rebuild/main/motion/probe coverage; `scripts/probe-output-color.mjs` hard-fails on lifecycle/velocity mode drift.
...
- Static `dist/` probes against `http://127.0.0.1:5180`: desktop/mobile output, desktop/mobile thumb, project-media, interactive mouse, and a narrow `/about/` smoke.

Decision: keep the `Fg` visibility and scroll-velocity ownership source-shaped. This closes one about-route lifecycle mismatch only; Phase 1 remains open for the auxiliary shader bridge, spotlight/thumb transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.

### S1-302 `XA/KA` Auxiliary Block Material Constructor State

This batch aligns the source constructor material state for about and floating auxiliary blocks. It does not replace the auxiliary shader bridge with source `jA/WA/YA/qA`, tune visual constants, change route data, or start Phase 2.

Source evidence:

- Source `XA` constructs the about auxiliary material with `dithering=true`, `transparent=true`, `envMapIntensity=.75`, `roughness=1`, `depthTest=false`, `renderOrder=10`, and `depthWrite=false`.
- Source `XA.customUniforms` includes `uMouse:new I(new Q)`, `uMouseFactor:new I(1)`, `uMouseLightness:new I(1)`, `uUvOffset:new I(new Q)`, `uUvOffsetScale:new I(1)`, `uReveal:new I(0)`, `uRevealSpread:new I(1)`, and `uScrollOpacity:new I(1)`.
- Source `KA` constructs the floating auxiliary material with the same standard material defaults but does not set `depthTest`, `depthWrite`, or `renderOrder`; those remain Three material defaults.
- Source `KA.customUniforms` includes the same `uMouse` / `uUvOffsetScale=1` defaults, plus `uRevealSpread:new I(10)`, `uRevealProject:new I(1)`, and `uRevealSides:new I(1)`.

Runtime and tooling changes:

- `createAuxiliaryBlockMaterial()` now gives auxiliary materials `uMouse:Vector2(0,0)` and `uUvOffsetScale=1`, matching the `XA`/`KA` constructor defaults.
- The about `XA` material keeps `depthWrite=false`, `depthTest=false`, and material `renderOrder=10`.
- The floating `KA` material no longer inherits about's depth overrides or `renderOrder`; runtime probe reports `depthWrite=true`, `depthTest=true`, and `renderOrder=null` for the unassigned material field.
- `__rogierOutputProbe.settings.work` now exposes separate `auxiliaryMaterial` and `floatingAuxiliaryMaterial` records with source mode labels, material flags, `uMouse`, and `uUvOffsetScale`.
- `scripts/probe-output-color.mjs` hard-fails if either auxiliary material drifts from the source constructor state.
- `scripts/audit-renderer-output.mjs` now extracts source `KA`, slices `XA`/`KA` constructor bodies, and records source/rebuild/probe evidence under `sourceManagers.auxiliaryBlockMaterials`.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `node --check scripts/probe-interactive-mouse.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-aux-material-audit.json` passed; the new `sourceManagers.auxiliaryBlockMaterials` audit subtree has no false/null values.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop and mobile output probes passed against `http://127.0.0.1:5177` with explicit `CHROME_PATH=/opt/google/chrome/chrome`; both asserted the new `XA`/`KA` material states.
- Desktop and mobile thumb spotlight probes passed against `http://127.0.0.1:5177`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining `5/5` visible media tracks on both pages.
- Static interactive mouse probe passed against `http://127.0.0.1:5180` with zero failures, exceptions, or console messages.
- Static `/about/` smoke passed against `http://127.0.0.1:5180/about/`: `aboutVisible=true`, `floatingVisible=true`, source `XA`/`KA` material modes present, no network failures, no runtime exceptions, and no console messages.

Decision: keep about/floating auxiliary material constructor state split by source `XA` versus `KA`. Do not restore the previous shared about-style depth/renderOrder overrides for floating, and do not move `uUvOffsetScale` back to the ordinary work ray scale without mirrored-bundle evidence. Phase 1 remains open because the auxiliary shader bridge, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals are still not closed.
...
This batch aligns where the environment material receives `dithering=true`. It changes only material construction ownership and QA guardrails; shader text, visual constants, route data, pass order, render-target topology, and runtime material values are otherwise unchanged.

Source evidence:

- Source `h1` constructs the environment material as `this.material=new u1({side:hn,envMapIntensity:Qn.ENVMAP_INTENSITY,fog:!1})`; the constructor object does not include `dithering`.
- Source `class u1 extends Ao` runs `constructor(e){super(e),this.dithering=!0,...}`.
- Therefore the source ownership is post-`super` assignment inside `u1`, not an `h1` constructor parameter.

Runtime and tooling changes:

- `createEnvironmentMaterial()` no longer passes `dithering:true` into `new MeshStandardMaterial(...)`.
- The rebuild now sets `material.dithering = true` immediately after construction and records `sourceDitheringOwnership = "source-u1-constructor-sets-dithering-after-super"`.
...
- `scripts/probe-output-color.mjs` hard-fails if the environment material loses the source ownership marker or if `dithering` reappears in the source `h1` constructor-params probe.
- `scripts/audit-renderer-output.mjs` checks source `u1` post-`super` assignment, source `h1` constructor-arg exclusion, rebuild post-construction assignment, rebuild constructor exclusion, and runtime probe coverage.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-env-dither-audit.json` passed; `sourceManagers.environmentU1.rebuildDitheringOwnership` reports source `u1` assignment, source `h1` constructor exclusion, rebuild post-construction assignment, rebuild constructor exclusion, and runtime probe coverage all true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop and mobile output probes passed for `/tmp/rd-env-dither-output-desktop` and `/tmp/rd-env-dither-output-mobile`; both reported `dithering=true`, `ditheringOwnershipMode=source-u1-constructor-sets-dithering-after-super`, and `constructorParamsIncludesDithering=false` with no browser failures/exceptions/console messages.
- Desktop and mobile thumb spotlight probes passed for `/tmp/rd-env-dither-thumb-desktop` and `/tmp/rd-env-dither-thumb-mobile`.
- Project media probe passed for `/tmp/rd-env-dither-project-media` and retained visible media tracks on the probed project pages.

Decision: keep the source `u1` post-construction environment dithering ownership. This closes one material-constructor attribution mismatch only; Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.

### S1-297 Source `Qe.gpuCheck()` GPU/LOW_RES Bridge

This batch replaces the rebuild-only `navigator.deviceMemory` / `connection.saveData` GPU-tier heuristic with the source-shaped async GPU check. It changes only GPU tier / `LOW_RES` ownership and QA guardrails; shader text, visual constants, route data, pass order, and render-target topology are unchanged.

Source evidence:

- Source `Le` defaults to `GPU_TIER=3` and `LOW_RES=false`.
- Source `Qe.init()` awaits `this.gpuCheck().catch(()=>{Le.GPU_TIER=3})` before the WebGL app managers initialize.
- Source `Qe.gpuCheck()` awaits the embedded detect-gpu helper, assigns `Le.GPU_TIER=e.tier`, catches errors with the same console message, then sets `Le.LOW_RES=Le.GPU_TIER<3`.
- The mirrored source rewrites detect-gpu 5.0.38 benchmark loading from unpkg to `/vendor/detect-gpu/benchmarks`.

Runtime and tooling changes:

- Added exact dependency `detect-gpu@5.0.38` and vendored the package's 16 benchmark JSON files under `public/vendor/detect-gpu/benchmarks`.
- `src/client/webgl.ts` now keeps module-level source defaults, exposes `initializeSourceGpuTier()`, calls `getGPUTier({ benchmarksURL: "/vendor/detect-gpu/benchmarks" })`, and preserves source fallback tier `3` / low-res `false` on errors.
- `src/client/main.ts` awaits `initializeSourceGpuTier()` before constructing `WebGLBackdrop`, so DPR, main fluid enablement, sky low-res time, and grid layer decisions read the initialized source bridge.
- `__rogierOutputProbe.renderer.gpuBridge` exposes mode, default mode, benchmark URL, tier, low-res, initialization status, detect-gpu result type, and fallback state.
- `scripts/probe-output-color.mjs` hard-fails if GPU bridge initialization, tier/low-res consistency, benchmark URL, DPR low-res, or main settings tier drift.
- `scripts/audit-renderer-output.mjs` checks source `Le/XD/Qe` anchors, mirror URL rewrite, exact rebuild bridge wiring, public benchmark files, and rejection of the old `deviceMemory/saveData` heuristic.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-gpu-tier-audit.json` passed; `sourceManagers.gpuTierBridge` reports source defaults/checks, rebuild checks, probe checks, old heuristic rejection, and 16 benchmark files.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop and mobile output probes passed for `/tmp/rd-gpu-tier-output-desktop` and `/tmp/rd-gpu-tier-output-mobile`; headless Chrome reported detect-gpu `FALLBACK` tier `1`, `LOW_RES=true`, `initialized=true`, and no browser failures/exceptions/console messages.
- Desktop and mobile thumb spotlight probes passed for `/tmp/rd-gpu-tier-thumb-desktop` and `/tmp/rd-gpu-tier-thumb-mobile`.
- Project media probe passed for `/tmp/rd-gpu-tier-project-media` and retained visible media tracks on the probed project pages.

Decision: keep the source-equivalent GPU/LOW_RES bridge. This closes the open GPU bridge-depth mismatch only; Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.

### S1-296 `p1` Root Scene Direct Child Order

This batch aligns the source `p1.init()` root scene direct-child order. It changes only hierarchy/add order and QA guardrails; it does not change shader formulas, visual constants, route data, pass order, render-target topology, or spotlight/thumb formulas.

Source evidence:

- Source `p1.setLights()` adds `ambientLight`, `spotLight`, `spotLight.target`, and `directionalLight` to the scene; it constructs `directionalLight2` but does not add it.
- Source `p1.init()` calls `setAboutBlocks()` and `setFloatingBlocks()` before the final `scene.add(sceneWrap)`.
- Source `setAboutBlocks()` and `setFloatingBlocks()` add the about and floating block groups directly to the scene.
- Therefore source root scene direct child order is lights, aboutBlocks, floatingBlocks, then `sceneWrap`; the prior rebuild inserted `sceneWrap` before about/floating blocks.

Runtime and tooling changes:

- The rebuild now adds `sceneWrap` to `homeScene` after `createAuxiliaryBlocks()`, preserving source root order while keeping the existing `sceneWrap -> blocksWrap/floor/env` child order.
- `__rogierOutputProbe.reflectionState.scene` now exposes `sourceRootChildOrderMode`, `sourceRootChildOrder`, and `sourceRootChildOrderMatches`.
- `scripts/probe-output-color.mjs` hard-fails if root scene child order drifts from `["ambientLight","spotLight","spotLight.target","directionalLight","aboutBlocks","floatingBlocks","sceneWrap"]`.
- `scripts/audit-renderer-output.mjs` checks source root-order anchors, rebuild add order, rejection of the old early `sceneWrap` add, and runtime probe coverage.
...
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-root-order-audit.json` passed; `sourceManagers.p1EnvironmentHierarchy.rootSceneDirectChildOrder` reports source order, rebuild order, old-order rejection, and runtime probe coverage all true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop and mobile output probes passed for `/tmp/rd-root-order-output-desktop` and `/tmp/rd-root-order-output-mobile`; both reported `sourceRootChildOrderMatches=true`.
- Desktop and mobile thumb spotlight probes passed for `/tmp/rd-root-order-thumb-desktop` and `/tmp/rd-root-order-thumb-mobile`.
- Project media probe passed for `/tmp/rd-root-order-project-media` and retained visible media tracks on the probed project pages.

Decision: keep source `p1` root direct-child order as lights -> aboutBlocks -> floatingBlocks -> sceneWrap. This closes one hierarchy/add-order mismatch only; Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.

### S1-295 `k1/O1/Lo` Displacement Target DPR Sizing

This batch aligns the source displacement/wavves render-target resize path. It changes only displacement target sizing and QA guardrails; it does not change shader formulas, visual constants, route data, pass order, project media, or render-target topology.

Source evidence:

- Source `k1.resize(e,t,n)` calls `this.renderManager.resize(t/10,t/10,n)`.
- Source `O1` extends `Lo`, so displacement resize flows through `Lo.resize(e,t,n)`.
- Source `Lo.resize(e,t,n)` rounds the passed dimensions after multiplying by DPR: `e=Math.round(e*n),t=Math.round(t*n)`.
- Therefore source displacement raw/composite target size is `round((height / 10) * dpr)`, not CSS-only `round(height / 10)`.

Runtime and tooling changes:

- The rebuild now sizes `displacementRawTarget` and `displacementTarget` with `Math.round((height / 10) * dpr)`.
- `__rogierOutputProbe.uniforms.passMaterials.displacement` now exposes `sizingMode`, `expectedTargetSize`, and `sourceDpr`.
- `scripts/probe-output-color.mjs` hard-fails if displacement raw/composite targets stop matching `height / 10 * sourceDpr`.
- `scripts/audit-renderer-output.mjs` checks source `k1 -> O1 -> Lo.resize` DPR evidence, rejects the old CSS-only size formula, and verifies runtime probe coverage.

Verification:

- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-displacement-size-audit.json` passed; `sourceManagers.Lo.rebuildDisplacementSizing` reports source DPR sizing, rebuild DPR usage, old CSS-only rejection, and runtime probe coverage all true.
- Desktop output probe passed with displacement target/raw target `90x90`: `/tmp/rd-displacement-size-output-desktop`.
- Mobile output probe passed with displacement target/raw target `84x84` at DPR `1`: `/tmp/rd-displacement-size-output-mobile`.
- Mobile DPR `1.25` output probe passed with displacement target/raw target `106x106`, proving the source DPR branch: `/tmp/rd-displacement-size-output-mobile-dpr125`.

Decision: keep displacement target sizing on source `k1.resize(height/10,height/10,dpr)` plus `Lo.resize()` DPR multiplication. Do not restore CSS-only `height / 10` sizing without mirrored-bundle evidence. Phase 1 remains open because this closes one target-sizing mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-294 `$1/j1/Lo` Media Clear Ownership Guardrail

This batch hardens source attribution for the project-media render-manager clear branch. It does not change production shader formulas, visual constants, route data, pass order, render targets, or render-target topology.

Source evidence:

- Source `Lo.initSettings()` sets `this.settings={renderToScreen:!0,clear:!1}`.
- Source `Lo.update(...)` does not inspect `this.settings.clear`.
- Source `j1.initSettings()` overrides settings as `{renderToScreen:!1,clear:!0}`, but that `clear` value is still not read by `Lo.update(...)`.
- Source `$1.update(e,t,n,i)` owns the active media clear behavior by temporarily setting `this.renderer.autoClear=!0`, calling `super.update(e,t,n,i)`, then restoring `this.renderer.autoClear=!1`.

Runtime and tooling changes:

- `renderMediaCompositeTarget()` keeps media target rendering inside a scoped `autoClear` branch and restores the previous renderer value afterward.
- `__rogierOutputProbe.passMaterials.mediaComposite` now exposes `renderManagerOwnership`, `settingsClearMode`, `autoClearMode`, and `rendererAutoClearRestored`.
- `scripts/probe-output-color.mjs` hard-fails if the media composite probe stops reporting source `$1.update()` clear ownership or fails to restore `renderer.autoClear`.
- `scripts/audit-renderer-output.mjs` extracts source `Lo`, `j1`, and `$1`, proves that `Lo.update()` does not consume `settings.clear`, checks source `$1.update()` auto-clear ownership, and checks the rebuild/probe guardrails.

Verification:

- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-media-clear-audit.json` passed; `sourceManagers.helperMaterialSurfaces.mediaClearOwnership` reports all source/rebuild/probe checks true, with only the known render-target/default Three false/null diagnostics remaining.
- `git diff --check` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with no browser failures/exceptions/console messages: `/tmp/rd-media-clear-output-desktop`.
- Mobile output probe passed with no browser failures/exceptions/console messages: `/tmp/rd-media-clear-output-mobile`.
- Desktop thumb spotlight probe passed and retained the existing thumb projection/state guardrails: `/tmp/rd-media-clear-thumb-desktop`.
- Project media probe passed and retained visible media tracks on the probed project pages: `/tmp/rd-media-clear-project-media`.

Decision: keep media clear ownership as source `$1.update()` temporary `renderer.autoClear` ownership, not as a consumed `j1.settings.clear` branch in `Lo.update()`. Phase 1 remains open because this is QA/guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.
...
### S1-291 `yg/U1/I1` Main Raw Camera Ef Surface Guardrail

This batch adds a source-backed runtime and audit guardrail for the main raw scene camera constructor and resize projection surface. It does not change shader text, render targets, pass order, route data, visual constants, or production camera formulas.

Source evidence:

- Source `function Ef(s,e,t,n=1)` computes perspective FOV as `2*Math.atan(s/e/(2*(t/n)))*(180/Math.PI)`.
- Source `yg` owns `this.distance=1e3`, computes `this.fov=Ef(innerWidth,innerWidth/innerHeight,this.distance)`, and constructs `this.camera=new Ya(this.fov,Pe.aspect,1,this.distance*2)`.
- Source `yg.setCamera()` sets the main raw camera position to `(0,0,this.distance)`.
- Source `yg.setCameraController(){}` is intentionally empty for this scene.
- Source `yg.resize(e,t,n)` calls `super.resize(e,t,n)`, then recomputes `this.camera.fov=Ef(e,e/t,this.distance)`, writes `this.camera.aspect=e/t`, and calls `this.camera.updateProjectionMatrix()`.
- Source `U1/I1` continues to render the main raw scene through the `yg` camera into `renderTargetA`.

Runtime and tooling changes:

- `__rogierOutputProbe.settings.main.mainRawCamera` now exposes `surfaceMode=source-yg-Ya-perspective-Ef-inner-aspect-near1-distance1000-far2000`.
- The same probe exposes `resizeProjectionMode=source-yg-resize-super-then-Ef-fov-aspect-updateProjectionMatrix`, expected/actual `fov/near/far`, source distance, position, aspect, and `fovMatchesSource`.
- `scripts/probe-output-color.mjs` now hard-fails if the main raw camera drifts from the source `Ef(...)` FOV formula, `near=1`, `far=2000`, distance `1000`, source position, or the source surface/resize markers.
- `scripts/audit-renderer-output.mjs` now extracts source `yg`, `Ef(...)`, and `U1/I1` raw-camera anchors under `sourceManagers.mainYgCameraSurface`, and checks rebuild/probe coverage.

Verification:

- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-main-yg-camera-audit-rerun.json` passed; `sourceManagers.mainYgCameraSurface` reports source, rebuild, and probe checks as true.
- `git diff --check` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed and confirmed the source `yg` camera surface and `Ef(...)` FOV parity: `/tmp/rd-main-yg-camera-output-desktop-rerun`.
- Mobile output probe passed with the same source camera surface and `Ef(...)` FOV parity: `/tmp/rd-main-yg-camera-output-mobile-rerun`.
- Desktop thumb spotlight probe passed and retained the thumb projection/state guardrails: `/tmp/rd-main-yg-camera-thumb-desktop-rerun`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-main-yg-camera-project-media-rerun`.

Decision: keep the main raw camera source-shaped as `yg -> Ya(Ef(...), Pe.aspect, 1, distance*2)` with distance `1000`, and keep resize projection ownership on the source `yg.resize()` FOV/aspect update path. Phase 1 remains open because this is camera-surface guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-289 `Se.settings` Scalar/Media No-Kill Ownership

This batch narrows the previously source-shaped `Se.settings` scalar/media state setter path to the source tween-ownership boundary. It removes rebuild-only tween registries and pre-emptive kills from source no-kill setters, while keeping the two source kill-owned setters kill-owned.

Source evidence:

- Source `Se.setDarken(e,t=.5)` writes directly only when `t===0`; otherwise it tweens `this.settings.darken` and updates `OA/CA.uDarken` from state, without storing or killing a darken tween.
- Source `Se.setSaturation(e,t=1.6)` and `Se.setContrast(e,t=1.6)` keep the same no-kill state-tween pattern for `uSaturation` and `C1/A1.uContrast`.
- Source `Se.showScene(e,t=1.6)` tweens `this.settings.sceneReveal` and writes `C1/A1.uReveal` from state, without a stored scene-reveal tween.
- Source `Se.setFluidStrength(e,t=.5)` and `Se.setMediaOpacity(e,t=1.6,n="expo.out",i=.25)` tween `this.settings.fluidStrength` / `this.settings.media.opacity` and update `C1/A1` uniforms from state, without stored tween ownership.
- Source `Se.setRevealSpread(e,t=1.6,n="power4.out")` and `Se.setEnvRotation(e,t=5.6)` explicitly kill their existing `revealSpreadAnim` / `envRotationAnim` before creating a replacement tween, so those two remain kill-owned in the rebuild.

Runtime and tooling changes:

- Removed rebuild-only scalar/media tween fields for saturation, contrast, darken, scene reveal, fluid strength, and media opacity.
- Removed `.kill()` and stored tween assignment from `showScene()`, `setDarken()`, `setSaturation()`, `setContrast()`, `setFluidStrength()`, and `setMediaOpacity()`.
- Kept `setRevealSpread()` and `setEnvRotation()` on stored kill-owned tween fields because the mirrored source owns those kills.
- `__rogierOutputProbe.settings.work.settingsStateOwnership` now exposes `scalarNoKillMode=source-no-kill-for-darken-saturation-contrast-showScene-fluidStrength-mediaOpacity` and `killOwnedMode=source-kill-owned-revealSpread-envRotation`.
- `scripts/probe-output-color.mjs` and `scripts/audit-renderer-output.mjs` hard-fail on scalar/media no-kill drift, missing kill-owned reveal/env rotation behavior, or reintroduced removed tween fields.

Verification:

- `node --check scripts/audit-renderer-output.mjs` passed.
- `node --check scripts/probe-output-color.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-settings-nokill-audit.json` passed; `sourceManagers.Se.settingsStateOwnership` reports `source=true` and `rebuild=true`.
- `git diff --check` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `settingsStateOwnership.scalarNoKillMode` and `killOwnedMode`: `/tmp/rd-settings-nokill-output-desktop`.
- Mobile output probe passed with the same settings ownership markers: `/tmp/rd-settings-nokill-output-mobile`.
- Desktop thumb spotlight probe passed and retained the thumb projection/state guardrails: `/tmp/rd-settings-nokill-thumb-desktop`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-settings-nokill-project-media`.

Decision: keep source no-kill scalar/media setters free of rebuild-owned tween registries and pre-emptive `.kill()` calls, while keeping reveal-spread and environment-rotation kill-owned. Phase 1 remains open because this is state ownership guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-288 `Se` Thumb State Setter No-Kill Ownership

This batch narrows the rebuild thumb state setters to the source `Se` ownership shape. It removes rebuild-only tween registries from the thumb darkness, thumb color, thumb saturation, and thumb mouse-lightness setter path while preserving source duration-zero direct writes and state-owned tween fan-out.

Source evidence:

- Source `Se.setThumbDarknessIntensity(e,t=1.6)` writes directly only when `t===0`; otherwise it tweens `this.settings.thumb.darknessIntensity` and updates `J.workThumbScene.renderManager.compositeMaterial.uniforms.uDarkenIntensity` from state on update.
- Source `Se.setThumbDarknessColor(e,t=1.6)` parses string colors through `sr(e)`, writes directly only when `t===0`; otherwise it tweens `this.settings.thumb.darknessColor` and updates thumb composite `uDarkenColor` from that state.
- Source `Se.setThumbSaturation(e,t=1.6)` writes directly only when `t===0`; otherwise it tweens `this.settings.thumb.saturation` and updates thumb composite `uSaturation` from state.
- Source `Se.setThumbMouseLightness(e,t=1.6)` writes directly only when `t===0`; otherwise it tweens `this.settings.thumb.mouseLightness` and fans out the state value to every work block material `uMouseLightness` on update.
- These source thumb setters do not keep `thumbDarknessTweens`, `thumbDarknessColorTweens`, `thumbSaturationTweens`, or `thumbMouseLightnessTweens`, and they do not kill previous thumb-state tweens before starting `oe.to(...)`.

Runtime and tooling changes:

- Removed the four rebuild-only thumb tween arrays and their `.kill()` / `.push(...)` paths.
- Kept the duration-zero direct assignments for thumb darkness, thumb color, thumb saturation, and thumb mouse-lightness because those branches are source-owned.
- Kept the tween targets on `thumbState` / `thumbState.darknessColor`, with onUpdate fan-out to thumb composite uniforms or work material `uMouseLightness`.
- `__rogierThumbProbe.thumbComposite.killMode` now reports `source-no-kill-for-thumb-state-setters`.
- `scripts/probe-thumb-spotlight.mjs` hard-fails on thumb-state kill-mode drift.
- `scripts/audit-renderer-output.mjs` now rejects the old thumb tween registries, `.kill()` calls, `.push(...)` storage, and missing thumb probe kill-mode assertion.
...
- `node --check scripts/probe-output-color.mjs` passed.
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-thumb-state-nokill-audit.json` passed; `sourceManagers.Se.thumbStateOwnership` reports `source=true` and `rebuild=true`.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed: `/tmp/rd-thumb-state-nokill-output-desktop`.
- Mobile output probe passed: `/tmp/rd-thumb-state-nokill-output-mobile`.
- Desktop thumb spotlight probe passed with `thumbComposite.killMode=source-no-kill-for-thumb-state-setters`: `/tmp/rd-thumb-state-nokill-thumb-desktop`.
- Mobile thumb spotlight probe passed with the same kill-mode marker and source mobile spotlight parallax branch: `/tmp/rd-thumb-state-nokill-thumb-mobile`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-thumb-state-nokill-project-media`.

Decision: keep the thumb `Se` setters state-owned and no-kill. Do not reintroduce rebuild-owned thumb tween registries, pre-emptive `.kill()` calls, or direct uniform tween targets without mirrored-bundle evidence. Phase 1 remains open because this is thumb state ownership guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-287 `Se.setBlocksColor` Work Material Emissive Ownership

This batch narrows the rebuild block-color setter to the source `Se` ownership shape. It removes the rebuild-only block-color tween registry and keeps `setBlocksColor()` scoped to every ordinary work block material emissive color.

Source evidence:

- Source `Se.setBlocksColor(e,t=1.6)` parses through `formatColor(e)`.
- Source `Se.setBlocksColor()` iterates `J.workScene.blocks.forEach(...)`.
- Each source iteration tweens only `i.instance.material.emissive` channels `r/g/b` with `ease:"expo.out"` and the supplied duration.
- Source does not keep a `blockColorTweens` array, kill previous block-color tweens, retarget only the active block, or write custom uniforms from this setter.

Runtime and tooling changes:

- Removed the rebuild `blockColorTweens` state.
- Changed `setBlocksColor()` to parse through `sourceRgbColor(value, DEFAULT_BG)` and start a GSAP tween on every `workItems[*].material.emissive`.
- Removed the duration-zero direct-copy branch from this source setter path so the runtime path stays shaped like source `oe.to(...)`.
- `__rogierOutputProbe.settings.work.blocksColorOwnership` now reports the source fan-out mode, target mode, no-kill mode, work-item count, active emissive color, and all-work emissive parity.
- `scripts/probe-output-color.mjs` and `scripts/audit-renderer-output.mjs` hard-fail on ownership drift, target drift, old tween registry state returning, active-only retargeting, direct uniform writes, or missing all-work emissive fan-out.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-blocks-color-audit.json` passed; `sourceManagers.Se.blocksColorOwnership` reports source/rebuild/probe coverage.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `blocksColorOwnership.mode=source-Se-setBlocksColor-tweens-all-work-material-emissive`, target mode `source-VA-MeshStandardMaterial-emissive`, no-kill mode, `workItemCount=10`, and `allWorkEmissiveMatchesActive=true`: `/tmp/rd-blocks-color-output-desktop`.
- Mobile output probe passed with the same block-color ownership markers: `/tmp/rd-blocks-color-output-mobile`.
- Desktop thumb spotlight probe passed and retained existing thumb/spotlight guardrails: `/tmp/rd-blocks-color-thumb-desktop`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-blocks-color-project-media`.

Decision: keep source `Se.setBlocksColor()` as a no-kill fan-out setter for ordinary work material emissive color only. Do not reintroduce a rebuild-owned tween registry, active-only emissive writes, custom-uniform retargeting, or direct-copy duration branch without mirrored-bundle evidence. Phase 1 remains open because this is state ownership guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-286 `Se.setAmbientLight` Ambient Setter Ownership

This batch narrows the rebuild ambient light setter to the source `Se` ownership shape. It removes the rebuild-only ambient tween bundle from the source setter path and keeps the background material out of `Se.setAmbientLight()` ownership.

Source evidence:

- Source `Se.init()` initializes `ambientColor:{r:0,g:0,b:0}` and `ambientIntensity:xt.ambient`.
- Source `Se.setAmbientColor(e,t=1.6)` parses through `formatColor(e)`, tweens `J.workScene.ambientLight.color`, and on every update writes `J.workScene.env.material.customUniforms.uDarkenColor` from the current ambient light color.
- Source `Se.setAmbientIntensity(e,t=1.6)` tweens `J.workScene.ambientLight.intensity`.
- Source `Se.setAmbientLight(e,t=.5,n=1.6)` only delegates to `setAmbientColor(e,n)` and `setAmbientIntensity(t,n)`.

Runtime and tooling changes:

- Replaced the single rebuild `setAmbientLight()` implementation with `setAmbientColor()`, `setAmbientIntensity()`, and a delegating `setAmbientLight()`.
- Removed `ambientTweens` and `currentAmbientIntensity`; the source ambient setters no longer kill previous color/intensity tweens.
- Stopped writing/tweening rebuild-only `backgroundMaterial.uniforms.uAmbientColor/uAmbientIntensity` from the source `Se` ambient setter path.
- `__rogierOutputProbe.settings.work.ambientOwnership` now reports the source delegate mode, color/intensity tween modes, no-kill mode, background-uniform non-ownership, ambient light color/intensity, environment darken color, and ambient/env color parity.
- `scripts/probe-output-color.mjs` and `scripts/audit-renderer-output.mjs` hard-fail on ownership drift, env fan-out drift, background setter retargeting, or old ambient tween state returning.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-ambient-audit.json` passed; `sourceManagers.Se.ambientOwnership` reports `source=true`, `rebuild=true`, and all probe anchors true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `ambientOwnership.mode=source-Se-setAmbientLight-delegates-color-intensity`, ambient light color matching environment darken color, and active `hashgraph-vc` ambient intensity `0.75`: `/tmp/rd-ambient-output-desktop`.
- Mobile output probe passed with the same ambient ownership markers: `/tmp/rd-ambient-output-mobile`.
- Desktop thumb spotlight probe passed and retained existing thumb/spotlight guardrails: `/tmp/rd-ambient-thumb-desktop`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-ambient-project-media`.

Decision: keep source `Se.setAmbientLight()` as a delegating setter only. `setAmbientColor()` owns ambient light color plus env `uDarkenColor` fan-out; `setAmbientIntensity()` owns ambient light intensity; rebuild-only background material uniforms are not source `Se` ambient targets. Phase 1 remains open because this is state ownership guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-285 `p1.setMouseFactor` Work Material Runtime Ownership

This batch adds a source-backed runtime guardrail for `p1.setMouseFactor()` fan-out to ordinary work `VA.uMouseFactor`. It does not change production rendering formulas, shader text, render targets, pass order, visual constants, route data, or Phase 1 closeout status.

Source evidence:

- Source `VA` constructs `uMouseFactor:new I(0)`.
- Source `p1.setMouseFactor(e)` writes `this.mouseF=e` and fans out `e` to every work block material via `t.instance.material.customUniforms.uMouseFactor.value=e`.
- Source `yD.animateIn()` calls `J.workScene.setMouseFactor(0)` and then tweens `mouseF:1` over `duration:3` with `onUpdate:()=>J.workScene.setMouseFactor(this.mouseF)`.
- Source work preview hover tweens `mouseF:.25` on enter and `mouseF:1` on leave, both through `J.workScene.setMouseFactor(this.mouseF)`.

Runtime and tooling changes:

- `__rogierOutputProbe.settings.work.mouseFactorOwnership` now reports the source ownership mode, constructor default, gallery-entry mode, preview enter/leave mode, source steady/preview targets, current state, active material uniform value, and whether every work material uniform matches state.
- `scripts/probe-output-color.mjs` waits for the source gallery-entry tween to settle, then hard-fails on mode/default/target drift, state-uniform mismatch, or per-work-material fan-out mismatch.
- `scripts/audit-renderer-output.mjs` now checks the mirrored `VA`, `p1.setMouseFactor`, `yD.animateIn`, and preview-hover anchors plus rebuild/probe coverage.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-mouse-factor-audit.json` passed; the remaining false diagnostics are the known render-target default/snapshot checks around `generateMipmaps`, `depthBuffer`, and `stencilBuffer`.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `mouseFactorOwnership.mode=source-p1-setMouseFactor-updates-VA-uMouseFactor`, `state=1`, `activeUniform=1`, and `allWorkUniformsMatchState=true`: `/tmp/rd-mouse-factor-output-desktop`.
- Mobile output probe passed with the same mouse-factor ownership markers: `/tmp/rd-mouse-factor-output-mobile`.
- Desktop thumb spotlight probe passed and retained existing thumb/spotlight guardrails: `/tmp/rd-mouse-factor-thumb-desktop`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-mouse-factor-project-media`.

Decision: keep `p1.setMouseFactor()` as the source-owned state fan-out path for ordinary work `VA.uMouseFactor`, including gallery entry target `1` and preview hover targets `.25` / `1`. Do not bypass this with direct per-material tweens or separate preview state without mirrored-bundle evidence. Phase 1 remains open because this is runtime ownership guardrail work only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals remain unresolved.

### S1-284 Helper Pass Shader Surface Parity

This batch aligned low-risk helper shader text surfaces against the mirrored source bundle. It does not change render-target topology, pass order, visual constants, route data, or Phase 1 closeout status.

Source evidence:

- Source `ig` FXAA uses the compact `FT/UT` vertex and fragment text surface, including source macro spacing, parameter names, and 4-space body indentation.
- Source `sg` luminosity helper uses `precision mediump float`, local `float v`, and `vec4(0)` in the fragment; its fullscreen vertex has source 4-space body indentation.
- Source `rg` bloom blur helper uses a dedicated vertex with `precision mediump float`, function name `blur`, local weight `w`, and source 4-space body indentation.
- Source `Na` standard blur and `cg` bloom composite share a fullscreen vertex without the extra blank line before `void main()`.
- Source `Ka/rA/oA` mouse simulation keeps compact float literals, source blank-line whitespace, and a trailing vertex-space surface.

Runtime and tooling changes:

- Split the previously shared fullscreen helper vertex into source-shaped variants for the `sg` and `rg` helper passes while keeping `Na/cg` on their source no-blank-line vertex surface.
- Replaced `homeLuminosityFragment`, `homeBloomBlurFragment`, `homeFxaaFragment`, `sourceFxaaVertex`, and `mouseSimulationFragment` with source-shaped text surfaces.
- Kept source-only Ka whitespace in runtime shader strings through interpolation helpers so the generated GLSL matches source without adding trailing whitespace to the TypeScript file.
- Updated `mouseSimulationProbe()` and `scripts/audit-renderer-output.mjs` to assert the new source-shaped Ka compact literals.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-helper-audit.json` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Shader dump passed at `/tmp/rd-helper-shader-dump`; `ig-fxaa`, `sg-luminosity`, `rg-bloom-blur`, `Na-standard-blur`, `cg-bloom-composite`, and `Ka-mouse-simulation` all report vertex delta `0` and fragment delta `0`.
- Desktop and mobile output probes passed with no browser failures/exceptions/console messages: `/tmp/rd-helper-output-desktop`, `/tmp/rd-helper-output-mobile`.
- Desktop and mobile thumb spotlight probes passed: `/tmp/rd-helper-thumb-desktop`, `/tmp/rd-helper-thumb-mobile`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-helper-project-media`.

Decision: keep the helper pass shader surfaces source-shaped and keep the updated QA anchors. This closes helper shader-text residuals only; Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.
...
| 221 | S1-296 | `p1` root scene direct child order | Source `p1.setLights()` adds ambient, spotlight, spotlight target, and the first directional light to the scene; source `p1.init()` then calls `setAboutBlocks()` and `setFloatingBlocks()` before the final `scene.add(sceneWrap)`. Source `setAboutBlocks()` and `setFloatingBlocks()` add those groups directly to the scene, so root scene order is lights -> aboutBlocks -> floatingBlocks -> sceneWrap. | Production shaders, pass order, render targets, visual constants, and route data are unchanged. The rebuild now adds `sceneWrap` after `createAuxiliaryBlocks()`, exposes `sourceRootChildOrderMode`, `sourceRootChildOrder`, and `sourceRootChildOrderMatches` in `reflectionState.scene`, and output probe plus renderer audit hard-fail on drift or restoration of the old early `sceneWrap` add. Desktop/mobile output probes, desktop/mobile thumb probes, and project-media probe passed for `/tmp/rd-root-order-*`. | Low | Keep as hierarchy/add-order parity only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 220 | S1-295 | `k1/O1/Lo` displacement target DPR sizing | Source `k1.resize(e,t,n)` calls `this.renderManager.resize(t/10,t/10,n)`, source `O1` extends `Lo`, and source `Lo.resize(e,t,n)` then runs `e=Math.round(e*n),t=Math.round(t*n)`. Source displacement raw/composite target size is therefore `round((height / 10) * dpr)`, not CSS-only `round(height / 10)`. | Production shaders and pass order are unchanged. `displacementRawTarget` and `displacementTarget` now use `Math.round((height / 10) * dpr)`. Runtime probe exposes `sizingMode`, `expectedTargetSize`, and `sourceDpr`; output probe hard-fails on target-size drift; renderer audit checks source `k1/O1/Lo` anchors, rebuild DPR usage, and rejection of the old CSS-only formula. Desktop, mobile, and mobile DPR `1.25` output probes passed for `/tmp/rd-displacement-size-*`. | Low | Keep as displacement target sizing parity only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 219 | S1-294 | `$1/j1/Lo` media clear ownership | Source `Lo.initSettings()` sets `clear:false`, source `j1.initSettings()` sets `clear:true`, but source `Lo.update(...)` does not inspect `this.settings.clear`; source `$1.update(e,t,n,i)` owns active media clear behavior by temporarily setting `renderer.autoClear=true`, calling `super.update(...)`, then restoring `renderer.autoClear=false`. | Production rendering topology is unchanged. `renderMediaCompositeTarget()` keeps `autoClear` scoped and restores the previous renderer value. `__rogierOutputProbe.passMaterials.mediaComposite` exposes source clear ownership markers, `scripts/probe-output-color.mjs` hard-fails on drift, and renderer audit checks source `Lo/j1/$1` anchors under `sourceManagers.helperMaterialSurfaces.mediaClearOwnership`. Full syntax/build/audit/browser probes passed for `/tmp/rd-media-clear-*`. | Low | Keep as `$1/j1/Lo` media clear ownership guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 218 | S1-291 | `yg/U1/I1` main raw camera `Ef` surface | Source `yg` owns `distance=1e3`, computes `fov=Ef(innerWidth,innerWidth/innerHeight,distance)`, constructs `new Ya(fov,Pe.aspect,1,distance*2)`, positions the camera at z `distance`, keeps `setCameraController(){}` empty, and on resize recomputes `fov=Ef(e,e/t,distance)`, writes `aspect=e/t`, and updates the projection matrix. | Production camera formulas are unchanged. `__rogierOutputProbe.settings.main.mainRawCamera` now exposes source surface/resize markers, expected/actual `fov/near/far`, distance, position, aspect, and `fovMatchesSource`. `scripts/probe-output-color.mjs` hard-fails on main raw camera drift, and renderer audit checks source `yg/Ef/U1/I1` anchors under `sourceManagers.mainYgCameraSurface`. Full syntax/build/audit/browser probes passed for `/tmp/rd-main-yg-camera-*`. | Low | Keep as `yg/U1/I1` main raw camera surface guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 217 | S1-290 | `p1/Ya` home camera surface | Source `p1.setCamera()` constructs `new Ya(55, innerWidth/innerHeight, 1, 2e3)` and sets camera position `(0,0,5.5)`. Source `Ya.resize()` only calls `updateProjectionMatrix()`, while source `Iu.resize()` owns the aspect write and second projection update. | Production camera formulas are unchanged. `__rogierOutputProbe.camera` exposes source surface/resize/initial-position markers plus expected/actual `fov/aspect/near/far` and viewport aspect parity. `scripts/probe-output-color.mjs` hard-fails on home camera drift, and renderer audit checks source `p1.setCamera`, `Ya`, and `Iu.resize` anchors. Full syntax/build/audit/browser probes passed for `/tmp/rd-home-camera-*`. | Low | Keep as `p1/Ya` home camera surface guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 216 | S1-289 | `Se.settings` scalar/media no-kill ownership | Source `Se.setDarken()`, `setSaturation()`, `setContrast()`, `showScene()`, `setFluidStrength()`, and `setMediaOpacity()` tween `this.settings` / `this.settings.media` and update uniforms from state without stored tween registries or pre-emptive kills. Source `setRevealSpread()` and `setEnvRotation()` explicitly kill their existing anim fields and remain kill-owned. | Rebuild now removes `saturationTween`, `contrastTween`, `darkenTween`, `sceneRevealTween`, `fluidStrengthTween`, and `mediaOpacityTween`, removes `.kill()` / stored assignment from the no-kill scalar/media setters, and keeps `setRevealSpread()` / `setEnvRotation()` kill-owned. `__rogierOutputProbe.settings.work.settingsStateOwnership` exposes `scalarNoKillMode` and `killOwnedMode`; output probe and renderer audit hard-fail on drift. Full syntax/build/audit/browser probes passed for `/tmp/rd-settings-nokill-*`. | Low | Keep as `Se.settings` scalar/media tween ownership guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 215 | S1-288 | `Se` thumb state setter no-kill ownership | Source `Se.setThumbDarknessIntensity()`, `setThumbDarknessColor()`, `setThumbSaturation()`, and `setThumbMouseLightness()` use source `t===0` direct-write branches, otherwise tween `this.settings.thumb` or `this.settings.thumb.darknessColor` and update thumb composite uniforms or work material `uMouseLightness` from state on update. The source keeps no thumb tween registries and does not kill previous thumb-state tweens. | Rebuild now removes `thumbDarknessTweens`, `thumbDarknessColorTweens`, `thumbSaturationTweens`, and `thumbMouseLightnessTweens`, keeps source duration-zero direct branches, and starts state-owned GSAP tweens without storage or `.kill()`. `__rogierThumbProbe.thumbComposite.killMode` exposes `source-no-kill-for-thumb-state-setters`; thumb probe and renderer audit hard-fail on drift. Full syntax/build/audit/browser probes passed for `/tmp/rd-thumb-state-nokill-*`. | Low | Keep as `Se` thumb state setter ownership guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 214 | S1-287 | `Se.setBlocksColor -> VA.material.emissive` ownership | Source `Se.setBlocksColor(e,t=1.6)` parses through `formatColor(e)`, iterates `J.workScene.blocks.forEach(...)`, and tweens each `i.instance.material.emissive` `r/g/b` channel with `ease:"expo.out"` and the supplied duration. It does not keep a block-color tween array, kill previous tweens, retarget only the active block, or write custom uniforms from this setter. | Rebuild now has `setBlocksColor()` parse through `sourceRgbColor(value, DEFAULT_BG)` and tween every `workItems[*].material.emissive` with source duration/ease. The old `blockColorTweens` state and duration-zero direct-copy branch are gone. `__rogierOutputProbe.settings.work.blocksColorOwnership` exposes source mode strings, work-item count, active emissive, and all-work emissive parity; `scripts/probe-output-color.mjs` and renderer audit hard-fail on drift. Full syntax/build/audit/browser probes passed for `/tmp/rd-blocks-color-*`. | Low | Keep as `Se` block-color setter ownership guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 213 | S1-286 | `Se.setAmbientLight -> setAmbientColor/setAmbientIntensity` ownership | Source `Se.setAmbientColor()` parses through `formatColor()`, tweens `J.workScene.ambientLight.color`, and fans out the current ambient light color to `J.workScene.env.material.customUniforms.uDarkenColor` on update. Source `Se.setAmbientIntensity()` tweens `J.workScene.ambientLight.intensity`. Source `Se.setAmbientLight(e,t=.5,n=1.6)` only delegates to those two setters and does not kill prior ambient tweens or write rebuild-only background material uniforms. | Rebuild now has source-shaped `setAmbientColor()`, `setAmbientIntensity()`, and delegating `setAmbientLight()`, with no `ambientTweens`/`currentAmbientIntensity` state and no background uniform writes in the source setter path. `__rogierOutputProbe.settings.work.ambientOwnership` exposes mode strings, ambient/env color parity, current ambient intensity, and background non-ownership; `scripts/probe-output-color.mjs` and renderer audit hard-fail on drift. Full syntax/build/audit/browser probes passed for `/tmp/rd-ambient-*`. | Low | Keep as `Se` ambient setter ownership guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 212 | S1-285 | `p1.setMouseFactor -> VA.uMouseFactor` runtime ownership | Source `VA` constructs `uMouseFactor` at `0`; source `p1.setMouseFactor(e)` owns `this.mouseF=e` and writes every work material `customUniforms.uMouseFactor.value=e`; source `yD.animateIn()` sets `0` then tweens to `1`; source preview hover tweens `.25` on enter and `1` on leave. | Production formulas are unchanged. `__rogierOutputProbe.settings.work.mouseFactorOwnership` now exposes the source mode/defaults/targets/current state/active uniform and all-work fan-out parity. `scripts/probe-output-color.mjs` hard-fails on drift, and renderer audit checks source `VA`, `p1.setMouseFactor`, `yD.animateIn`, preview-hover, rebuild, and probe anchors. Full syntax/build/audit/browser probes passed for `/tmp/rd-mouse-factor-*`. | Low | Keep as `p1 -> VA` state fan-out guardrail only. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 211 | S1-284 | helper pass shader surface parity | Source helper shaders `ig`, `sg`, `rg`, `Na`, `cg`, and `Ka/rA/oA` have distinct text surfaces in the mirrored bundle, including precision qualifiers, helper names, compact numeric literals, whitespace, and per-pass fullscreen vertex variants. | Rebuild helper shader strings now dump source-shaped for all six helper passes. `sourceFullscreenVertex` was split only where the source bundle has distinct vertex text; Ka source whitespace is generated without TypeScript trailing whitespace; output probe and renderer audit now check the compact source Ka anchors. Shader dump `/tmp/rd-helper-shader-dump` reports `v 0 / f 0` for the six helper passes. | Low | Keep as helper shader-text parity and QA surface cleanup. This does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 210 | S1-283 | `U1/I1` lensflare mouse input guardrail | Source `U1.onMouseMove({x:e,y:t})` calls `this.renderManager&&this.renderManager.setLightPosition(0,1-t/Pe.h)`. Source `I1.setLightPosition(e,t)` writes `this.lensflareMaterial.uniforms.uLightPosition.value.set(e,t)` only when `this.settings.lensflare.enabled`, preserving the default disabled lensflare no-op path. | The rebuild now calls `setMainLensflareLightPosition(0, 1 - event.clientY / window.innerHeight)` from the shared window mousemove handler and guards the write with `SOURCE_MAIN_LENSFLARE_SETTINGS.enabled && mainLensflareMaterial`, so default visuals stay unchanged while the source input path exists if lensflare is enabled. `__rogierOutputProbe.uniforms.lensflare` exposes `setLightPositionMode`, `mouseMoveInputMode`, and disabled no-mutation state; `scripts/probe-output-color.mjs` and renderer audit hard-fail on drift. | Low | Keep this as source `U1/I1` input-path evidence only. It does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 209 | S1-282 | `mg/GA/$A` rounded block geometry parity | Source `class mg extends Vt` is a custom rounded-box `BufferGeometry`, not Three examples `RoundedBoxGeometry`: default `radiusSegments` is `1`, `new mg(e,e,e,.05)` clamps radius to `.05`, allocates `((r+1)*r+1)<<3` vertices, writes indexed `position/normal/uv`, and sets every UV to `.5,.5`. Source `GA.createCube()` and `$A.createCube()` both use `new mg(e,e,e,.05)`, while source `ZA.createCube()` uses `new Cr(e,e,e)` and must stay box geometry. | The rebuild now uses a local source-shaped `sourceRoundedBoxGeometry()` for ordinary work blocks and about `$A` blocks, producing the source default `24` vertices and `132` indices with centered UVs. Floating `ZA` blocks remain `BoxGeometry(0.5,0.5,0.5)`. `__rogierOutputProbe.settings.work` exposes `activeGeometry`, `aboutGeometry`, and `floatingGeometry`; `scripts/probe-output-color.mjs` hard-fails on geometry count, radius, indexing, UV-center, or `ZA` box drift; renderer audit checks source `mg/GA/$A/ZA` anchors plus rebuild/probe coverage. `git diff --check`, script syntax checks, renderer audit, build, desktop/mobile output probes, desktop/mobile thumb spotlight probes, and project-media probe passed; output probes confirmed work/about `position/normal/uv=24`, `index=132`, `radius=.05`, `radiusSegments=1`, `uvAllCenter=true`, and floating `BoxGeometry`, with no browser failures/exceptions/console messages. | Low | Keep this as source geometry-input parity for `HA/zA` and spotlight/thumb transfer diagnostics. It does not close Phase 1 visual parity, remaining spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 208 | S1-281 | `GA.createPlane()` ray-plane scale parity | Source `GA.createPlane()` sets `rayPlane.scale` to `(35*1.3,23*1.3,1)`, then calls `rayPlane.scale.multiplyScalar(t)` with `t=1.5`; source therefore scales the ray plane on x/y/z before adding it to `rotationWrap`. | Production ray plane z scale now matches source: `createWorkRayPlane()` sets z to `MOUSE_RAY_SCALE` instead of leaving it at `1`. `__rogierOutputProbe.mouseSimulation.active` exposes `sourceRayPlaneScale=[68.25,44.85,1.5]`, and `sourceShape.rayPlaneScaleMatchesSource` now checks x/y/z. `scripts/probe-output-color.mjs` hard-fails on ray-plane z drift, and renderer audit checks the mirrored `multiplyScalar(t)` anchor plus rebuild/probe coverage. `git diff --check`, script syntax checks, renderer audit, build, and desktop/mobile output probes passed; both browser probes confirmed ray plane scale `[68.25,44.85,1.5]` with no failures/exceptions/console messages. | Low | Keep this as `GA.createPlane()` ray/raycast geometry parity. It does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 207 | S1-280 | `A1/C1` renderer-audit source surface cleanup | Source `A1` uses `FragColor`, source `blend(1,...)` / `blend(11,...)`, `mixed.rgb *= uContrast`, source noise mixes on `mixed.rgb`, source `displacementUv` / `vignetteF` locals, and does not include `#include <tonemapping_fragment>`. | Production rendering is unchanged. `scripts/audit-renderer-output.mjs` now checks the current source-shaped `A1` surface instead of stale bridge-era strings such as `gl_FragColor`, `sourceBlend(...)`, and `color`. It also records source/rebuild absence of `tonemapping_fragment` and positive `sourceSurfaceChecks` for `displacementUv`, `vignetteF`, and noise-sample order. Renderer-audit recursive false output dropped from `18` to `8`; the remaining false entries are render-target default/snapshot diagnostics, not `A1` shader-surface residuals. | Low | Keep this as QA harness cleanup only. It reduces false attribution noise before the next production Phase 1 batch, but it does not close spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
| 206 | S1-279 | `V1/k1` sky/displacement post-render state guardrail | Source `V1.update(e,t,n,i)` calls `super.update(Le.LOW_RES?0:e,t,n,i)` and only then writes `z1.uTime=Le.LOW_RES?0:e`; source `k1.update(e,t,n,i)` calls `super.update(e,t,n,i)` and only then writes `N1.uTime=e`. Source `k1.init()` also owns the raw wavves scene background as `new Color("red").convertLinearToSRGB()`. | Intended production visual behavior is unchanged. The rebuild now centralizes `SOURCE_SKY_BACKGROUND` and `SOURCE_DISPLACEMENT_BACKGROUND`, exposes `source-V1-super-update-before-z1-uTime-write`, `source-k1-super-update-before-N1-uTime-write`, source sky background parity, and source red displacement raw-scene background parity through `__rogierOutputProbe`. The output probe hard-fails on those markers, and renderer audit checks source anchors plus rebuild function order `render -> setRenderTarget(null) -> uTime write`. `git diff --check`, syntax checks, renderer audit, build, desktop/mobile output probes, and desktop thumb smoke passed with no browser failures/exceptions/console messages. | Low | Keep this as source sky/displacement state-order and raw-background evidence. It does not close Phase 1 visual parity, spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, or floor/environment residuals. |
...
| 198 | S1-271 | `sA/C1` mouse-sim ping-pong binding guardrail | Source `sA` constructs two FBOs with `current=0` and `output=fbos[0]`; each `render()` binds `uTexture` to `fbos[current].texture`, flips `current=1-current`, sets `output=fbos[current]`, then renders to that output. Source `nD.init()` samples `J.workScene.renderManager.mouseSimulation.bufferSim.output.texture` once for `C1/A1.tMouseSim` before the render loop, so `C1.tMouseSim` should remain the initial output texture and only equal the current ping-pong output on frames where the output index is back to zero. | Production rendering is unchanged. `__rogierOutputProbe.uniforms.preComposite.materialUniformSurface.tMouseSimSourceBinding` now exposes source binding mode, source `sA` initial output index, source flip mode, target count, bound texture index, current output index, whether the binding remains the initial output texture, and whether current-output equality only occurs when the output index is zero. `scripts/probe-output-color.mjs` hard-fails on drift, and renderer audit checks source `sA` ping-pong anchors plus rebuild/probe coverage. `node --check` checks, `ASTRO_TELEMETRY_DISABLED=1 npm run build`, renderer audit, desktop/mobile output probes, desktop thumb probe, and project-media probe passed; `gc-2026` and `hashgraph-vc` retained `5/5` visible media tracks. | Low | Keep this as a runtime/QA guardrail. It strengthens the transfer-chain evidence for `nD/C1/Ka/sA` but does not close any Phase 1 visual blocker. Continue production Phase 1 from spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment source residuals. |
| 197 | S1-270 | Output probe source-state stable wait | Source `Se` owns light and scalar state through tweened `settings` objects, so a browser probe must sample after those source-state writes have settled. A default fixed 5.2s wait can be too early on slower cold starts and can report transient `lightStateDirectional` / `lightStateDirectional2` mismatches even though the source-shaped state reaches `directionalLight=1.5`, `directionalLight2=1`, `spotLight=220`, and `sceneReveal=1`. | Production rendering is unchanged. `scripts/probe-output-color.mjs` now reads page summary through `readProbeSummary()` and waits after the base delay until `__rogierOutputProbe` reports entered body state, source light-state ownership/matching, source spot/directional/directional2 values, source scalar settings ownership, uniform parity, and `sceneReveal=1`, bounded by `PROBE_STABLE_TIMEOUT` (default `30000`). The existing hard assertions remain unchanged. `node --check scripts/probe-output-color.mjs`, desktop output probe, mobile output probe, thumb spotlight probe, `node --check scripts/audit-renderer-output.mjs`, and renderer audit passed. | Low | Keep output probes state-aware instead of relying only on fixed sleep. This is QA harness stability only; it does not close any Phase 1 visual blocker. Continue production Phase 1 from spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment source residuals. |
...
| 192 | S1-265 | Rejected debug shortcut retirement | Earlier Phase 1 attribution proved `debug-composite-transfer=*`, `debug-spotlight-map=off`, and `debug-thumb-colorspace=*` are diagnostic shortcuts without mirrored-source support for production parity. Source `SD.init()` assigns the thumb composite target as `SpotLight.map`, and source `OA/CA` / Three renderer evidence does not support shader-level gamma transfer or query-driven thumb target color-space mutation. | The current runtime no longer accepts `debug-composite-transfer=*`, no longer exposes `uDebugTransferMode`, no longer allows `debug-spotlight-map=off`, and no longer mutates thumb target color spaces through `debug-thumb-colorspace=*`. `homeSpotlightMap()` always returns `thumbCompositeTarget.texture`. Attribution scripts no longer construct those rejected URLs, while keeping non-rejected stage/darken/lighten, floor/environment, renderer-output, and raw-work-composite diagnostics. Renderer audit now checks the runtime and QA scripts for `noRejectedCompositeTransferDebug`, `noRejectedSpotlightMapOffDebug`, `noRejectedThumbColorspaceDebug`, and `homeSpotlightMapAlwaysSourceThumbComposite`. | Low | Keep rejected shortcuts retired; do not reintroduce them as production or debug paths without new mirrored-source evidence. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity. |
| 191 | S1-264 | `C1/Se/I1` fluid-strength constructor/runtime ownership | Source `C1` constructs `uFluidStrength:new I(.5)`, but source `Se.init()` initializes `settings.fluidStrength=0`. Source `Se.setFluidStrength(e,t=.5)` is the later runtime writer to `J.mainScene.renderManager.compositeMaterial.uniforms.uFluidStrength`, and source `I1.update()` gates the fluid simulation update from the current `C1.uFluidStrength` uniform before binding the fluid target. | The rebuild now constructs `C1/A1` with `SOURCE_C1_FLUID_STRENGTH_DEFAULT=0.5` and records `source-Se-setFluidStrength-writes-C1-uFluidStrength` plus `source-C1-constructor-0_5-Se-settings-initial-0` on material `userData`. The Home and WorkGallery `.5` compensation setter calls are removed, the frame loop no longer rewrites `uFluidStrength` from `settingsState.fluidStrength`, and the main fluid update gate reads the current `C1` uniform. Output probes expose the intended initial divergence (`settingsState.fluidStrength=0`, `C1.uFluidStrength=.5`) while preserving `Se.setFluidStrength()` state-owned runtime writes. Renderer audit checks the source constructor anchor, source `Se` setter anchor, rebuild constructor/default markers, no per-frame rewrite, no Home/Gallery compensation setter, and C1-uniform fluid update gate. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, and project-media probe passed; desktop/mobile output probes confirmed `settingsState.fluidStrength=0`, `C1.uFluidStrength=0.5`, and `fluidStrengthUniformMatchesState=false`, while `gc-2026` and `hashgraph-vc` project media retained `5/5` visible tracks with no failures/exceptions/console messages. | Low-medium | Keep `C1.uFluidStrength` constructor-owned at `.5` until `Se.setFluidStrength()` writes it. Do not force initial parity with `settingsState.fluidStrength=0`; that state/uniform divergence is source-shaped. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity. |
| 190 | S1-263 | `T1/w1/SD/p1` mobile thumb spotlight guardrail | Source `SD.init()` binds `J.workScene.spotLight.map=J.workThumbScene.renderManager.renderTargetComposite.texture`, sets spotlight position `(0,0,3.7)`, target `(0,0,-8)`, and intensity `220`. Source `p1.update()` applies `this.spotLight.position.x=this.camera.position.x*.175`, and its Y branch is desktop `this.camera.position.y*.175` vs mobile `.3+this.camera.position.y*.175`. Source `T1.resize(e,t,n)` calls `this.renderManager.resize(t,t,1)`, so the thumb raw/composite targets are square CSS-height targets at DPR `1`. | No production runtime formula changed. `scripts/probe-thumb-spotlight.mjs` now accepts `VIEWPORT=desktop|mobile`, derives expected spotlight Y branch and thumb target size from the selected viewport, records viewport metadata, and asserts desktop `900x900` / mobile `844x844` thumb targets. Renderer audit reads the thumb probe script and now checks source mobile parallax evidence plus rebuild/probe mobile branch coverage. Desktop and mobile thumb probes passed; mobile confirms `parallaxYOffsetMode=source-p1-mobile-0_3-plus-camera-y-parallax`, `mobileYOffset=0.3`, spotlight position `[0,0.3,3.7]`, and no browser failures/exceptions/console messages. | Low | Keep the thumb/spotlight probe running in both desktop and mobile when touching `SD/p1/T1/w1` projection, spotlight, or thumb target code. This is guardrail/evidence hardening only; continue Phase 1 from actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity. |
| 189 | S1-262 | `VA/GA` `uUvOffset` shader declaration/runtime bridge | Source `HA` declares `uniform vec3 uUvOffset;`, computes `vec2 mouseUv = newUv + uUvOffset.xy`, and divides by `uUvOffsetScale`. Source `VA.customUniforms` creates `uUvOffset:new I(new Q)` with a `Vector2`, and source `GA.createPlane()` writes only `this.material.customUniforms.uUvOffset.value.x` and `.y`, plus `uUvOffsetScale`. The source therefore combines a raw shader `vec3` declaration with runtime Vector2/XY ownership. | The rebuild now declares `uniform vec3 uUvOffset;` in ordinary work `workBlockSourceHaVertexShader` and the shared work `workBlockVertexPars`, while keeping `uUvOffset: { value: sourceMouseUvOffset() }` as a runtime Vector2. Output probe exposes/asserts `uvOffsetShaderDeclaration=source-HA-vec3-uUvOffset`, `uvOffsetRuntimeBridgeMode=source-VA-uniform-value-Vector2-GA-writes-xy-shader-reads-xy`, and source-shape booleans. Renderer audit checks source shader/runtime anchors plus rebuild shader/probe coverage; shader dump confirms `VA-work` vertex/fragment delta `0` and `sourceUvOffsetBeforeRevealUniforms` matched. `git diff --check`, syntax checks, build, renderer audit, shader dump, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low-medium | Keep ordinary work shader text on source `vec3` while preserving the source runtime Vector2/XY bridge. Do not coerce the runtime uniform to `Vector3`, or revert the shader declaration to `vec2`, without mirrored-bundle evidence. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity. |
| 188 | S1-261 | `Xt/a1/o1` floor normal texture object binding | Source `Xt.loadTexture=e=>this.textureLoader.load(e)` returns the `Texture` object synchronously. `Xt.preloadTextures()` assigns that object to `Xt.floorNormal`, applies repeat wrapping, and source `a1.init()` awaits `Xt.floorNormal`, sets `repeat.set(45,45)`, then constructs `o1` with that same normal-map object. The rebuild previously constructed the floor material with a placeholder normal map and replaced `tNormalMap` only after the image load callback resolved. | The rebuild now has `loadTextureImmediate()` returning both the immediate `Texture` object and the load-completion promise. After source WebP extension selection, floor normal setup binds `floorMaterial.uniforms.tNormalMap` and `uMapTransform` to the immediate floor-normal object before the load promise resolves, while the promise only marks `sourceTexturePreloadState.floorNormal=true`. Output probe asserts `objectBindingMode=source-Xt-loadTexture-immediate-texture-object-bound-before-onload`, `uniformIsImmediateTexture=true`, `loadedSameImmediateTexture=true`, repeat `[45,45]`, and empty color space. Renderer audit checks source `Xt.loadTexture`, `Xt.preloadTextures()`, source `a1` floor setup, the rebuild immediate-loader helper, and binding order. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low-medium | Keep floor-normal uniform binding on the immediate `TextureLoader.load()` object and use the promise only as the preload gate. Do not move back to placeholder-then-replace semantics without mirrored-bundle evidence. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one floor texture lifecycle mismatch only. |
| 187 | S1-260 | `o1` floor material uniform order | Source `o1` constructs base uniforms in this order: `tReflect`, `uMapTransform`, `uMatrix`, `uColor`, `uReflectivity`, `uMirror`, `uFloorMixStrength`, `uNormalDistortionStrength`. When `normalMap` is supplied, source then appends `tNormalMap` and `uNormalScale` before reassigning the existing `uMapTransform` slot from the normal texture matrix. The rebuild had the same active fields, but declared `tNormalMap` before the reflection scalar uniforms, which drifted from the source constructor surface. | The rebuild now declares floor material uniforms in source order and records `SOURCE_O1_FLOOR_UNIFORM_KEYS`, runtime `Object.keys(floorMaterial.uniforms)`, and `matchesSourceOrder` in the output probe. `scripts/probe-output-color.mjs` asserts that order; `scripts/audit-renderer-output.mjs` checks source `o1` order, rebuild factory order, and probe coverage. This is a constructor-surface alignment only; formulas, target wiring, floor constants, normal-map binding, reflector blur, and visual tuning are unchanged. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low | Keep `o1` floor material uniforms in source order. Do not move `tNormalMap` before the reflection scalar uniforms without mirrored-bundle evidence. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one floor constructor-surface mismatch only. |
| 186 | S1-259 | `lA` main composite bool constructor ownership | Source `lA` constructs `uniforms:{tScene:new I(null),tBloom:new I(null),tBlur:new I(null),tFluid:new I(null),tMouseSim:new I(null),boolBloom:new I(!1),boolFluid:new I(!1),boolLuminosity:new I(!1),boolFxaa:new I(!1)}`. Source `Lu.update()` writes those four bool uniforms from `this.settings` only before rendering the `lA` composite screen. The rebuild's `createMainCompositeMaterial()` had initialized those bools directly from `sourceMainRenderSettings`, so `boolFluid` could be constructor-true on GPU tier 3 even though source `lA` constructor defaults are false. | The rebuild now constructs the `lA/aA` source-surface material with all four bool uniforms as `false`, records constructor defaults and `source-Lu-update-writes-lA-bools-from-settings-before-composite-render` in material `userData`, and no longer copies main settings into the constructor. Output probe exposes/asserts constructor-false defaults, source runtime-write ownership, and current false values for this source-surface artifact. Renderer audit checks source `lA` constructor defaults, source `Lu.update()` runtime writes, rebuild constructor defaults, old settings-in-constructor absence, and probe coverage. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low | Keep `lA` bool uniforms constructor-false. Do not reintroduce constructor-time `sourceMainRenderSettings` bools without mirrored-bundle evidence. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one source-surface constructor ownership mismatch only. |
| 185 | S1-258 | `sg` luminosity constructor settings ownership | Source `sg` constructs `uniforms:{tMap:new I(null),uThreshold:new I(1),uSmoothing:new I(1)}`. Source `Lu.initRenderer()` and `I1.initRenderer()` each create `new sg`, then write `uThreshold` and `uSmoothing` from `this.settings.luminosity` after construction. The rebuild had constructed luminosity materials with the final settings values directly, moving source `Lu/I1.initRenderer()` ownership into the constructor. | The rebuild now constructs luminosity materials with `tMap:null`, `uThreshold=1`, and `uSmoothing=1`, records constructor threshold/smoothing plus `source-Lu-I1-initRenderer-writes-sg-threshold-smoothing-after-construction` on material `userData`, then writes final settings values after construction. Output probe exposes/asserts constructor defaults, init-settings ownership, and final work/main threshold/smoothing parity. Renderer audit checks source `sg` constructor defaults, source `Lu/I1.initRenderer()` settings writes, rebuild constructor defaults followed by settings writes, and probe coverage. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low | Keep `sg` luminosity materials constructed with threshold/smoothing defaults of `1` and leave final threshold/smoothing values to source `Lu/I1.initRenderer()`-style settings writes after construction. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one helper-material constructor/settings ownership mismatch only. |
| 184 | S1-257 | `OA/Lu` work composite bool constructor ownership | Source `OA` constructs `boolBloom`, `boolFluid`, `boolLuminosity`, and `boolFxaa` with `new I(!1)`, even though source `kA.initSettings()` enables work bloom/luminosity. Source `Lu.update()` later writes those four `OA` uniforms from `this.settings` immediately before rendering the work composite screen. The rebuild had initialized `boolBloom` and `boolLuminosity` directly from work settings in `createCompositeMaterial()`, which moved runtime `Lu.update()` ownership into the constructor. | The rebuild now constructs all four `OA` bool uniforms as `false`, records the constructor bool defaults on material `userData`, and keeps the existing runtime writes from `renderSettings` immediately before work composite rendering. Output probe exposes/asserts constructor-false defaults, `source-Lu-update-writes-OA-bools-from-settings-before-composite-render`, and runtime bools matching work settings (`bloom=true`, `fluid=false`, `luminosity=true`, `fxaa=false`). Renderer audit checks source/rebuild constructor defaults and source/rebuild runtime writes. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low | Keep `OA` bool uniforms constructor-false and owned by the `Lu.update()` runtime write before work composite render. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one constructor/runtime ownership mismatch only. |
| 183 | S1-256 | `M1` thumb material uniform declaration order | Source `M1` constructs uniforms in this order: `tMap`, `uResolution`, `uMapSize`, `uProgress`, `uTransitionCount`, `uTransitionSmoothness`. The rebuild had the same fields and constructor defaults after S1-255, but declared `uMapSize` before `uResolution`, which is a source-structure drift in the thumb material constructor surface. | The rebuild now declares thumb material uniforms in the source order and records `Object.keys(material.uniforms)` into the thumb probe. Thumb probe asserts the exact source order, and renderer audit checks the source/rebuild order through `orderedIncludes(...)` while keeping constructor-null and post-`setImage` binding checks. `git diff --check`, syntax checks, build, renderer audit, thumb spotlight probe, desktop/mobile output probes, and project-media probe passed; project media retained `5/5` visible tracks. | Low | Keep `M1` uniform declaration order as `tMap`, `uResolution`, `uMapSize`, `uProgress`, `uTransitionCount`, `uTransitionSmoothness`. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one constructor-surface ordering mismatch only. |
| 182 | S1-255 | `M1/E1` thumb material constructor and image binding ownership | Source `M1` constructs thumb material uniforms with `tMap:new I(null)`, `uResolution:new I(new Q)`, and `uMapSize:new I(new Q)`. Source `E1.setImage(e)` waits for `Xt.thumbsReady`, awaits `Xt.getProjectThumbById(e).src`, assigns the resulting texture to `this.material.uniforms.tMap.value`, then sets both `uMapSize` and `uResolution` to `1,1`. The source does not construct `M1` with a placeholder texture or prefilled `1,1` vectors. | The rebuild now constructs `M1` thumb materials with `tMap:null`, zero-size `uMapSize` / `uResolution` vectors, and constructor evidence in material `userData`, while `setSourceThumbImage()` still performs the source post-`thumbsReady` texture binding and `1x1` writes. Thumb probe exposes/asserts constructor mode, constructor `tMap` null state, constructor vector sizes `[0,0]`, and post-`setImage` `mapBound=true` / `mapSize=[1,1]` / `resolution=[1,1]`. Renderer audit checks source `M1/E1` anchors, rebuild source-null constructor defaults, and rejects the old `this.placeholder` plus constructor `Vector2(1,1)` bridge scoped to `createThumbPlane()`. `git diff --check`, syntax checks, build, renderer audit, thumb spotlight probe, desktop/mobile output probes, and project-media probe passed; project media retained `5/5` visible tracks. | Low | Keep `M1` constructor defaults source-null/zero-size and leave texture plus `1x1` sizing to the source `E1.setImage()` lifecycle. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one thumb lifecycle ownership mismatch only. |
| 181 | S1-254 | `Se` main/media background color state tween ownership | Source `Se.init()` initializes `settings.mainColor={r:0,g:0,b:0}` and `settings.media.background={r:0,g:0,b:0}`. Source `setMainColor(e,t)` writes duration-zero color through `this.settings.mainColor`, and otherwise tweens `this.settings.mainColor` before writing every `.c-color` element from state in `onUpdate` using source `Fn` rounding. Source `setMediaBackground(e,t)` writes/tweens `this.settings.media.background` and fans out to `J.mediaScene.mediaItems.backgroundColor` from state. The source does not tween each DOM element as the state object and does not keep a standalone media-background state outside `Se.settings.media.background`. | The rebuild now keeps `settingsState.mainColor` and `settingsState.media.background`, writes `.c-color` DOM styles through `sourceMainColorCss()` from `settingsState.mainColor`, and updates `mediaBackground` plus each media plane `uBackgroundColor` from `settingsState.media.background`. The rebuild-only per-element `mainColorTweens`, `mediaBackgroundTweens`, and standalone `mediaBackgroundState` are removed. Output probes expose/assert `mainColorElementsMatchState`, `mediaBackgroundMatchesState`, and `mediaPlaneBackgroundsMatchState`. Renderer audit checks source `Se.setMainColor` / `setMediaBackground` anchors, rebuild state-owned paths, and rejects the old direct DOM tween / standalone media state paths. `git diff --check`, syntax checks, build, renderer audit, desktop/mobile output probes, thumb spotlight probe, and project-media probe passed; project media retained `5/5` visible tracks. | Low-medium | Keep `Se` main color and media background color state-owned through `settingsState.mainColor` and `settingsState.media.background`, with DOM/media fan-out from state updates. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes another state tween ownership slice only. |
| 180 | S1-253 | `VA/XA` work material `uCoords` runtime ownership | Source `VA.update(e,t,n,i)` writes `this.customUniforms.uTime.value=e` and `this.customUniforms.uCoords.value.set(Pe.w*i,Pe.h*i)`. Source about-block `XA.update(e,t,n,i)` uses the same `uCoords` write. Source `p1.resize(e,t,n)` passes `Math.min(n,1.5)` into the work-scene resize/update path, so `i` is the capped work DPR and `Pe.w/Pe.h` are CSS viewport dimensions. This differs from rounded render-target size at non-integer DPR. Source floating block `KA.update()` only writes `uTime`, so it is not part of this ownership change. | The rebuild now computes source work viewport coords as `window.innerWidth/window.innerHeight * sourceWorkDpr()` and applies them through `setSourceWorkMaterialUCoords()` in both resize and visible-work update paths. The about auxiliary material follows the same source `XA` path; the floating auxiliary material keeps its existing resize-owned `uCoords` behavior. Output probes expose/assert `sourceUCoordsMode=source-VA-update-Pe-width-height-times-capped-dpr-no-render-target-rounding`, `sourceUCoordsExpected`, and `uCoordsMatchesSource`, while leaving `uCoordsMatchesWorkTarget` as diagnostic. Renderer audit extracts source `VA` and `XA`, checks rebuild ownership markers, and rejects the old rounded work-target writes. `git diff --check`, syntax checks, build, renderer audit, desktop output probe, mobile DPR `1.25` output probe, and thumb spotlight probe passed. The DPR `1.25` mobile probe confirmed expected source coords `[487.5,1055]` while the render target was `488x1055`, so source-match is true and work-target-match is false as expected. | Low-medium | Keep ordinary work and about `XA` material `uCoords` sourced from CSS viewport times capped work DPR, not rounded render-target dimensions. Do not apply this to floating `KA` unless mirrored-bundle evidence shows `KA.update()` owning `uCoords`. Continue Phase 1 from actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution parity; this closes one runtime ownership mismatch only. |
...
| 81 | S1-154 | `GA/VA` ordinary work UV offset type | Source ordinary `VA` binds `uUvOffset:new I(new Q)` where `Q` is a Vector2, and source `GA.createPlane()` only writes `.x`/`.y` plus `uUvOffsetScale=t`; this is separate from auxiliary block shader surfaces that contain `uniform vec3 uUvOffset`. | Superseded by S1-262. This earlier batch correctly established the runtime `Vector2`/XY ownership, but its temporary `uniform vec2 uUvOffset` shader declaration is no longer current. Production ordinary work now keeps runtime `Vector2` values while declaring source `uniform vec3 uUvOffset;` and reading `uUvOffset.xy`, matching the combined source shader/runtime evidence. | Low-medium | Keep the S1-154 runtime attribution, but use S1-262 as the current shader declaration rule. Continue Phase 1 from remaining ordinary work material/projection feel, thumb-map transfer/content, floor/environment distribution, or source-backed `OA/CA` transfer evidence. |
...
| `GA/VA` ordinary blocks | Direct source-shaped `HA/zA` with documented runtime bridge | Ordinary work now uses direct source-shaped `HA/zA` templates, source material defaults, source alpha/reveal tail, source world/UV paths, spotlight-map projection guardrails, source `SPECULAR` macro with inactive `PHYSICAL` proof, and source `uniform vec3 uUvOffset;` shader text. The remaining ordinary-work source bridge is runtime-only: source `VA` constructs a `Vector2`, source `GA` writes `.x/.y`, and source `HA` reads `uUvOffset.xy`; projection/material feel still differs. | Yes for strict 1:1 | Keep the runtime `uUvOffset` bridge documented and continue from source-backed projection/material feel, thumb-map transfer/content, floor/environment distribution, and `OA/CA` transfer evidence. |
...
- Source `Le.LOW_RES` starts false and is later set by GPU tier. This non-fix note was superseded by S1-297, which added the source-equivalent detect-gpu 5.0.38 bridge and removed the local `sourceLowRes()` approximation.
...
| --- | --- | --- | --- |
| `TD` about scroll opacity | `TD.onScroll()` updates spotlight every RAF and sets mobile `uScrollOpacity = Cs(scroll, 0, Pe.h * .25, 1, 0, true)`, while desktop stays `1`. | `updateAuxiliaryBlocks()` updates the about spotlight while visible and applies the same desktop/mobile opacity shape from the page-scroll state, with a window scroll fallback. | Treat as implemented and guarded by the about smoke. |
...
| S1-07 | `TD/Fg` scroll lifecycle | About `TD` opacity/spotlight lifecycle is source-shaped. Floating `Fg` now owns visibility on `animateIn` `onStart` / `animateOut` `onComplete`, and `ZA` z update receives the source page-scroll velocity feed by accumulating `.005 * abs(scroll.velocity)` into `translationZ` only while the about floating blocks are visible, with entry/destroy resets to avoid cross-route carryover. | Guarded by renderer audit, output probe lifecycle checks, and a static `/about/` smoke confirming `aboutVisible=true`, `floatingVisible=true`, `scrollVelocitySource=page-scroll-velocity`, and no browser failures/exceptions/console. |
...
- At that time, added a local `sourceGpuTier()` bridge and made `sourceLowRes()` derive from that same tier gate; this was later replaced by the source-equivalent detect-gpu bridge in S1-297.
...
### S1-280 `A1/C1` Renderer-Audit Source Surface Cleanup

This batch updates renderer-audit attribution only. It does not change production WebGL code, shader formulas, render targets, textures, runtime probes, visual constants, or browser behavior.

Source evidence:

- Source `A1` uses GLSL3 `FragColor`, not the older `gl_FragColor` bridge string.
- Source `A1` calls the source dispatcher as `blend(1,...)` and `blend(11,...)`, not the older rebuild helper name `sourceBlend(...)`.
- Source `A1` keeps the active variable name `mixed.rgb` through contrast and noise mixing, including `mixed.rgb *= uContrast` and the two noise mixes on `mixed.rgb`.
- Source `A1` keeps `vec2 displacementUv = ...` and `float vignetteF = ...` locals.
- Source `A1` does not include `#include <tonemapping_fragment>`.

Tooling changes:

- `scripts/audit-renderer-output.mjs` now checks `FragColor`, `blend(1`, `blend(11`, `mixed.rgb *= uContrast`, and the source-shaped noise mix strings in `rebuildA1.checks`.
- The audit now records `tonemappingFragmentAbsent` for both source and rebuild `A1`.
- The old negative `inertSourceComputations` object was replaced with positive `sourceSurfaceChecks` for `displacementUvMatchesSource`, `vignetteFMatchesSource`, and `noiseSampleOrderMatchesSource`.

Verification:

- `git diff --check` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-a1-audit-cleanup-final.json` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Recursive audit false output dropped from `18` to `8`; the remaining false entries are render-target default/snapshot diagnostics (`generateMipmaps`, `depthBuffer`, and `stencilBuffer`), not `A1` shader-surface residuals.

Decision: keep the renderer audit aligned with the current source-shaped `A1/C1` surface before using it for the next production Phase 1 batch. This is QA harness cleanup only; Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment residuals.

### S1-279 `V1/k1` Sky/Displacement Post-Render State Guardrail

This batch strengthens source runtime/audit guardrails for the sky and wavves/displacement `Lo` subclasses. It intentionally avoids screenshot-led tuning and does not change shader formulas, render-target topology, spotlight constants, project data, gallery math, or visual constants.

Source evidence:

- Source `V1.update(e,t,n,i)` calls `super.update(Le.LOW_RES?0:e,t,n,i)` before writing `this.renderManager.compositeMaterial.uniforms.uTime.value=Le.LOW_RES?0:e`.
- Source `k1.update(e,t,n,i)` calls `super.update(e,t,n,i)` before writing `this.renderManager.compositeMaterial.uniforms.uTime.value=e`.
- Source `k1.init()` assigns `this.backgroundColor=new ye("red").convertLinearToSRGB()` and then `this.scene.background=this.backgroundColor`.
- Source `V1.init()` similarly owns `#666666` as the sky raw scene background through `convertLinearToSRGB()`.

Runtime and tooling changes:

- `src/client/webgl.ts` now centralizes `SOURCE_SKY_BACKGROUND` and `SOURCE_DISPLACEMENT_BACKGROUND`.
- `__rogierOutputProbe.textures.skyComposite` exposes `backgroundExpected`, `backgroundMatchesSource`, and `uTimeUpdateOrder=source-V1-super-update-before-z1-uTime-write`.
- `__rogierOutputProbe.passMaterials.displacement` exposes `backgroundMode=source-k1-red-linear-to-srgb`, `backgroundExpected`, `backgroundMatchesSource`, and `uTimeUpdateOrder=source-k1-super-update-before-N1-uTime-write`.
- `scripts/probe-output-color.mjs` hard-fails on sky/displacement background or post-render `uTime` order drift.
- `scripts/audit-renderer-output.mjs` checks the source `V1/k1` anchors and verifies rebuild function order `render -> setRenderTarget(null) -> uTime write` for both `renderSkyTarget()` and `renderDisplacementTarget()`.
...
- `node --check scripts/probe-output-color.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-sky-displacement-order-audit-pre.json` passed; recursive false/null review stayed at the expected `19` source-negative/default entries.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with sky/displacement post-render `uTime` order and background parity, and no failures/exceptions/console messages: `/tmp/rd-sky-displacement-order-output-desktop`.
- Mobile output probe passed with the same guardrail and no failures/exceptions/console messages: `/tmp/rd-sky-displacement-order-output-mobile`.
- Desktop thumb smoke passed with no failures/exceptions/console messages: `/tmp/rd-sky-displacement-order-thumb-desktop`.

Decision: keep explicit sky/displacement background and post-render state-order guardrails. Do not move `z1/N1.uTime` writes before their source `Lo` render-manager passes, or remove the source red wavves raw background, without mirrored-bundle evidence. Phase 1 remains open because this is state-order/target-content evidence only; remaining spotlight/thumb transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.
...
This batch adds a source-backed runtime/audit guardrail for the thumb scene surface and `x1` render-manager settings. It intentionally avoids screenshot-led tuning and does not change project data, shader formulas, spotlight constants, gallery scroll math, work/main render-manager settings, or route media behavior.

Source evidence:

- Source `T1.init()` sets the thumb scene background from `new ye("#222222").convertLinearToSRGB()` and assigns it to `this.scene.background`.
- Source `T1.setCamera()` constructs `new Kn(-1,1,1,-1,0,1)`.
- Source `T1.setRenderManager()` constructs `new x1(this.renderer,this.scene,this.camera)`.
- Source `T1.resize(e,t,n)` calls `this.renderManager.resize(t,t,1)`, keeping the square thumb target ownership.
- Source `x1.initSettings()` assigns `this.settings={renderToScreen:!1}`.
- Source `Lo.update()` renders the composite screen to the composite target when `renderToScreen` is false, then calls `renderer.setRenderTarget(null)`.

Runtime and tooling changes:

- `src/client/webgl.ts` now centralizes `SOURCE_THUMB_BACKGROUND="#222222"` and assigns `thumbScene.background` through `sourceLinearToSrgbColor(SOURCE_THUMB_BACKGROUND)`.
- The thumb orthographic camera remains `OrthographicCamera(-1,1,1,-1,0,1)` and is now exposed through runtime probe expected/actual camera bounds.
- `thumbRenderSettings` records source `x1` settings as `{ renderToScreen:false }`.
- `renderThumbTargets()` now follows the source branch shape: render to screen only when `thumbRenderSettings.renderToScreen` is true; otherwise render into `thumbCompositeTarget` and reset the renderer target to `null`.
- `__rogierThumbProbe.thumbSceneSurface` exposes source mode, expected/actual background, background match, expected/actual camera bounds, camera match, and expected/actual `x1` `renderToScreen` settings.
- `scripts/probe-thumb-spotlight.mjs` hard-fails on background, camera, or thumb render-manager settings drift.
- `scripts/audit-renderer-output.mjs` checks source `T1/x1/Lo` anchors plus rebuild/probe coverage for the background, camera, settings, and false-branch render target reset.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-thumb-scene-audit-final.json` passed; recursive false/null review stayed at the expected `19` source-negative/default entries.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop thumb probe passed with `thumbSceneSurface.mode=source-T1-background-camera-x1-renderToScreen-settings`, source background/camera/settings parity, and no failures/exceptions/console messages: `/tmp/rd-thumb-scene-desktop`.
- Mobile thumb probe passed with the same source scene-surface/settings parity and no failures/exceptions/console messages: `/tmp/rd-thumb-scene-mobile`.
- Desktop output probe passed after the thumb scene/settings change with no failures/exceptions/console messages: `/tmp/rd-thumb-scene-output-desktop`.
- Mobile output probe passed after the thumb scene/settings change with no failures/exceptions/console messages: `/tmp/rd-thumb-scene-output-mobile`.

Decision: keep this as source `T1/x1` thumb scene/settings ownership evidence/guardrail only. It proves the thumb background conversion, camera bounds, render-manager `renderToScreen:false` settings, and composite-target false branch, but it does not close any remaining visual residual. Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment distribution evidence.

### S1-276 `Lu/kA/I1` Render-Manager Init Settings Guardrail

This batch adds a source-backed runtime/audit guardrail for render-manager init settings ownership. It intentionally avoids screenshot-led tuning and does not change shader formulas, transfer functions, render-target topology, route media behavior, spotlight constants, or visual brightness settings.

Source evidence:

- Source `Lu.initSettings()` owns the render-manager base defaults: `renderToScreen=false`, FXAA disabled, mousesim disabled, luminosity threshold `.1` / smoothing `1` disabled, bloom strength `.05` / radius `.01` disabled, blur scale `1` / strength `8` disabled, and fluid disabled with `mouseForce=25`, `cursorSize=15`, `delta=.019`, `poissonIterations=1`, `bounce=false`.
- Source `kA.initSettings()` overrides work settings: mousesim enabled, luminosity threshold `.1` / smoothing `.95` enabled, bloom strength `.15` / radius `1.5` enabled, blur disabled at scale `1` / strength `8`, and fluid disabled with `mouseForce=25`, `cursorSize=20`, `delta=.019`, `poissonIterations=1`, `bounce=false`.
- Source `I1.initSettings()` owns main settings: `renderToScreen=true`, FXAA/mousesim/luminosity/bloom/blur disabled, fluid `enabled=Le.GPU_TIER>=3` with `mouseForce=5`, `cursorSize=6`, `delta=.125`, `poissonIterations=1`, `bounce=false`, and disabled lensflare defaults `scale=new Q(1.5,1.5)`, `exposure=1`, `clamp=1`.

Runtime and tooling changes:

- `src/client/webgl.ts` now clones source render settings per render-manager instance with `cloneSourceRenderSettings(...)` instead of sharing the source template object.
- `sourceRuntimeMainRenderSettings()` derives main fluid enablement from the local source GPU-tier bridge, preserving the source `Le.GPU_TIER>=3` branch.
- `__rogierOutputProbe.settings.work.renderManagerSettings` exposes source mode, expected settings, actual settings, source-match boolean, and instance-owned clone status for the work `kA` settings.
- `__rogierOutputProbe.settings.main.renderManagerSettings` exposes the same source-match surface for main `I1` settings plus expected fluid enablement.
- `__rogierOutputProbe.settings.main.lensflareSettings` exposes expected/actual `scale`, `exposure`, `clamp`, and `enabled` defaults for source `I1` lensflare settings.
- `scripts/probe-output-color.mjs` hard-fails on settings mode drift, expected/actual settings drift, instance-owned clone drift, main fluid branch drift, or lensflare default drift.
- `scripts/audit-renderer-output.mjs` now checks source `Lu/kA/I1` settings anchors plus rebuild helper, field, and probe coverage.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-settings-audit-final.json` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with exact source work settings, exact source main settings, source GPU-tier fluid expectation, disabled lensflare defaults, and instance-owned work/main settings clones: `/tmp/rd-settings-output-desktop`.
- Mobile output probe passed with the same settings and lensflare parity: `/tmp/rd-settings-output-mobile`.
- Desktop thumb spotlight smoke passed with existing progress/order, thumb wrapping, image lifecycle, target sizing, source project order, and spotlight projection guardrails intact: `/tmp/rd-settings-thumb-desktop`.

Decision: keep this as source `Lu/kA/I1` render-manager settings ownership evidence/guardrail only. It proves settings templates, runtime main fluid branch, lensflare defaults, and per-instance clones, but it does not close any remaining visual residual. Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment distribution evidence.
...
This batch adds a source-backed runtime guardrail for `p1` carousel/light scalar ownership. It does not intentionally change production visual behavior, shader formulas, render-target ownership, route behavior, or visible constants.

Source evidence:

- Source `p1.init()` initializes `count`, `theta`, and `itemWidth`.
- Source `p1.setBlocks()` derives `this.radius=Math.round(this.itemWidth/2/Math.tan(Math.PI/this.count))`.
- Source `p1.setBlocks()` then derives `this.lightRadius=this.radius-3.5`.
- Source `p1.setBlocks()` uses the source theta/radius state for block distribution and `sceneWrap.position.z=radius-.3`.
- Source `p1.setLights()` sets `this.maxSpotLightIntensity=220`.

Runtime and tooling changes:

- `WebGLBackdrop` now stores `count`, `theta`, `itemWidth`, `radius`, and `lightRadius` as source-shaped scalar state.
- `createWorkScene()` assigns `count=cards.length`, `theta=360/count`, `itemWidth=6.5`, `radius=Math.round(itemWidth/2/Math.tan(Math.PI/count))`, and `lightRadius=radius-3.5`.
- Carousel layout now uses `this.theta`, and the empty-card path resets the stored scalar state.
- `p1UpdateCulling.sourceCarouselDistribution` now exposes actual/expected item width, count, theta, radius, lightRadius, and source-match booleans.
- The lights probe now exposes `maxSpotLightIntensity` and `maxSpotLightIntensityMatchesSource`.
- `scripts/probe-output-color.mjs` hard-fails if the new carousel/light scalar fields drift from the mirrored source.
- `scripts/audit-renderer-output.mjs` checks mirrored `p1.init()`, `p1.setBlocks()`, and `p1.setLights()` anchors plus rebuild/probe coverage for the scalar state.
...
- `node --check scripts/probe-output-color.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-p1-source-state-audit.json` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `actualItemWidth=6.5`, `actualCount=10`, `actualThetaDegrees=36`, `actualRadius=10`, `actualLightRadius=6.5`, `lightRadiusMatchesSource=true`, `maxSpotLightIntensity=220`, and `maxSpotLightIntensityMatchesSource=true`: `/tmp/rd-p1-source-state-output-desktop`.
- Mobile output probe passed with the same scalar source parity: `/tmp/rd-p1-source-state-output-mobile`.
- Desktop thumb spotlight probe passed with existing progress/order and projection guardrails intact, `3/9` in-map spotlight samples, and nonzero map luma: `/tmp/rd-p1-source-state-thumb-desktop`.
- Mobile thumb spotlight probe passed with existing progress/order and projection guardrails intact, `3/9` in-map spotlight samples, nonzero map luma, and the source mobile `0.3` spotlight Y branch: `/tmp/rd-p1-source-state-thumb-mobile`.
- Project media probe passed for `gc-2026` and `hashgraph-vc`; both retained visible media tracks with no failures/exceptions/console messages: `/tmp/rd-p1-source-state-project-media`.

Decision: keep this as source `p1` scalar-state evidence/guardrail only. It proves `count`, `theta`, `itemWidth`, `radius`, `lightRadius`, and `maxSpotLightIntensity` ownership, but it does not close any remaining visual residual. Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment distribution evidence.

### S1-272 `p1` Scene Background/Fog Guardrail

This batch adds a source-backed runtime guardrail for the `p1.init()` scene surface. It does not intentionally change production visual behavior, shader formulas, render-target ownership, route behavior, or visible constants.

Source evidence:

- Source `p1.init()` creates `this.fog=new bu("grey",0,100)` and assigns `this.scene.fog=this.fog`.
- Source `p1.init()` computes `this.backgroundColor=new ye(BA.BACKGROUND_COLOR).convertLinearToSRGB()` and assigns `this.scene.background=this.backgroundColor`.
- Source `BA.BACKGROUND_COLOR` is `#1a1a1a`.
- Source `a1` constructs `o1({ color:"#4a4a4a", normalMap:e, uMirror:1, reflectivity:.97, uFloorMixStrength:15 })`, so the floor material does not enable the `USE_FOG` branch at construction.
- Source `h1` constructs `u1({ side:BackSide, envMapIntensity:1, fog:false })`, so the environment material explicitly opts out of fog.

Runtime and tooling changes:

- `SOURCE_WORK_FOG_COLOR` now centralizes the source `grey` fog color used by the Home work scene.
- `__rogierOutputProbe.settings.work.sceneSurface` now exposes the source background mode, source background color, runtime background array, `backgroundMatchesSource`, source fog mode, explicit `fogType`, fog color/near/far, `fogMatchesSource`, `floorMaterialFogBranch`, and `environmentMaterialFog`.
- `scripts/probe-output-color.mjs` hard-fails if `p1` scene background/fog ownership drifts, if the source background/fog values no longer match, or if floor/environment fog branches become enabled.
- `scripts/audit-renderer-output.mjs` now checks the mirrored `p1.init()` scene background/fog anchors plus rebuild and probe assertions for this scene surface.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-p1-scene-surface-audit-final.json` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop output probe passed with `sceneSurface.backgroundMatchesSource=true`, `fogMatchesSource=true`, `floorMaterialFogBranch=false`, and `environmentMaterialFog=false`: `/tmp/rd-p1-scene-surface-output-desktop`.
- Mobile output probe passed with the same scene-surface assertions: `/tmp/rd-p1-scene-surface-output-mobile`.
- Desktop thumb spotlight probe passed with existing source spotlight/thumb projection guardrails intact: `/tmp/rd-p1-scene-surface-thumb`.
- Project media probe passed for `gc-2026` and `hashgraph-vc`; both retained `5/5` visible media tracks with no failures/exceptions/console messages: `/tmp/rd-p1-scene-surface-project-media`.

Decision: keep this as a source `p1.init()` scene-surface guardrail. It proves the Home scene background/fog ownership and prevents future fog-branch drift in floor/environment materials, but it does not close the hard horizon/fog-bed residual. Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment distribution evidence.

### S1-271 `sA/C1` Mouse-Sim Ping-Pong Binding Guardrail

This batch strengthens the existing `nD/C1` main mouse-simulation binding evidence. It does not change production rendering, shader formulas, visual constants, render targets, or route behavior.

Source evidence:

- Source `sA` constructs two FBOs as `[this.fbo, this.fbo.clone()]`, initializes `current=0`, and initializes `output=this.fbos[0]`.
- Source `sA.render()` binds `uTexture` to `this.fbos[this.current].texture`, flips `current=1-this.current`, assigns `output=this.fbos[this.current]`, then renders to that output target.
- Source `nD.init()` binds `J.mainScene.renderManager.compositeMaterial.uniforms.tMouseSim.value` to `J.workScene.renderManager.mouseSimulation.bufferSim.output.texture` once before the render loop.
- Because `nD.init()` samples the initial `sA.output` texture and does not rebind every frame, `C1/A1.tMouseSim` should remain the initial output texture. It should match the current ping-pong output only on frames where the current output index is `0`.

Runtime and tooling changes:

- `__rogierOutputProbe.uniforms.preComposite.materialUniformSurface.tMouseSimSourceBinding` now exposes the source binding mode, source initial output index, source flip mode, target count, bound texture index, current output index, initial-texture parity, and the current-output match rule.
- `scripts/probe-output-color.mjs` hard-fails if `C1.tMouseSim` stops using the source one-time initial output binding, if the ping-pong target count/index metadata drifts, or if current-output equality is reported on the wrong ping-pong index.
- `scripts/audit-renderer-output.mjs` now checks the mirrored `sA` ping-pong anchors plus rebuild and output-probe coverage for the binding semantics.
...
- `node --check scripts/probe-output-color.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-mousesim-pingpong-audit.json` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
...
- Desktop thumb spotlight probe passed with existing projection guardrails intact: `/tmp/rd-mousesim-pingpong-thumb`.
- Project media probe passed for `gc-2026` and `hashgraph-vc`; both retained `5/5` visible media tracks with no failures/exceptions/console messages: `/tmp/rd-mousesim-pingpong-project-media.json`.

Decision: keep this as a source binding/runtime guardrail for `nD/C1/Ka/sA`. It strengthens transfer-chain evidence but does not close a Phase 1 visual blocker. Phase 1 remains open for spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment source residuals.

### S1-270 Output Probe Source-State Stable Wait

This batch hardens the Home output probe against false early failures. It does not change production rendering, source constants, shader formulas, render-target setup, or route behavior.

Source evidence:

- Source `Se` owns light intensities through state/tween setters, with `J.workScene.directionalLight`, `directionalLight2`, and `spotLight` updated from that state.
- The source-shaped final Home state for the default active route expects `spotLight=220`, `directionalLight=1.5`, `directionalLight2=1`, matching light objects, and `sceneReveal=1`.
- The output probe's fixed wait can run before those source-state tweens settle on cold browser starts, producing transient `lightStateDirectional` and `lightStateDirectional2` failures that disappear under a longer wait.

Runtime and tooling changes:

- `scripts/probe-output-color.mjs` now reads the CDP page summary through `readProbeSummary()`.
- After the existing base `PROBE_WAIT`, the script polls `__rogierOutputProbe` until the source state has settled or `PROBE_STABLE_TIMEOUT` expires.
- The settle condition checks entered body state, source light-state ownership, light-object/state parity, source spot/directional/directional2 values, source scalar settings ownership, uniform parity, and `sceneReveal=1`.
- Existing hard assertions are unchanged, so real source-state drift still fails after the stable-wait window.
...
- Thumb spotlight probe passed: `/tmp/rd-probe-stable-thumb`.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs > /tmp/rd-probe-stable-audit.json` passed.

Decision: keep output probes state-aware rather than relying only on fixed sleeps. This is QA harness stability only; Phase 1 remains open for actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and remaining floor/environment source residuals.
...
This batch aligns source `i1` floor-reflector constructor defaults. It does not change floor shader formulas, visual constants, project data, source `i1.setSize()` CSS sizing, or final composite transfer behavior.

Source evidence:

- Source `i1` constructor defaults to `width=512`, `height=512`, `clipBias=0`, and `blurIterations=2`.
- Source `i1` constructs `renderTarget=new Dn(width,height,{depthBuffer:false})`, then clones `renderTargetRead` and `renderTargetWrite`, then toggles the raw target `depthBuffer=true`.
- Source `i1` initializes `renderTargetUniform` from `blurIterations > 0 ? renderTargetRead.texture : renderTarget.texture`.
- Source `i1` creates `t1` and immediately sets `blurMaterial.uniforms.uResolution.value.set(width,height)`.
- Source `i1.update()` derives the blur loop count from `this.blurIterations` and computes `d=(blurIterations-u-1)*15`, giving default directions `(15,0)` then `(0,0)`.

Runtime and tooling changes:

- `floorReflectionTarget` now constructs at source `512x512` instead of local `1x1`, before read/write target cloning and the existing raw-depth toggle.
- `floorReflectionBlurIterations` now stores source default `2`, and the blur loop reads this field instead of hardcoding the loop count.
- `floorReflectionRenderTargetUniform` now initializes through the source `blurIterations > 0` conditional.
- `floorReflectionBlurMaterial.uResolution` now starts as an empty `Vector2`, then gets the source constructor resolution `512,512` in the reflector initialization path before later resize ownership writes CSS viewport dimensions.
- `__rogierOutputProbe` now exposes/asserts source constructor mode, constructor width/height, blur iteration count, positive-blur initial read texture, constructor blur resolution, and expected blur directions.
- `scripts/probe-output-color.mjs` asserts those constructor/default fields in both quick reflection checks and detailed floor checks.
- `scripts/audit-renderer-output.mjs` now checks the source `i1` constructor/default anchors plus rebuild and probe coverage.
...
- `node --check scripts/probe-output-color.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-i1-constructor-audit.json`; `sourceManagers.i1` source checks, rebuild checks, and rebuild probe checks were all true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop `scripts/probe-output-color.mjs` passed with no browser failures/exceptions/console messages; runtime reported `constructorWidth=512`, `constructorHeight=512`, `blurIterations=2`, `renderTargetUniformConstructorMode=source-i1-positive-blurIterations-initial-read-texture`, `blurConstructorResolution=[512,512]`, and `blurExpectedDirections=[[15,0],[0,0]]`.
- Mobile `scripts/probe-output-color.mjs` passed with the same source constructor/default fields and no browser failures/exceptions/console messages.
- Desktop `scripts/probe-thumb-spotlight.mjs` passed with source spotlight/thumb guardrails intact, `3/9` in-map samples, and nonzero `mapLumaMean`.
...
- Source `p1.init()` creates `blocksWrap` and `sceneWrap`.
- Source `p1.init()` calls `sceneWrap.add(blocksWrap)` before creating and attaching floor/environment content.
- Source `p1.init()` creates `floor=this.add(a1)` and sets `floor.position.y=-1.65`.
- Source `p1.init()` creates `env=this.add(h1)` and sets `env.position.y=-12.65`.
- Source `p1.init()` sets `env.rotation.y=-Xc(this.rotationAdjustment)`.
- Source `p1.init()` then calls `sceneWrap.add(floor)`, `sceneWrap.add(env)`, and `scene.add(sceneWrap)`.
- Source `setBlocks()` derives `rotationAdjustment` from the `demorgen` work index.

Runtime and tooling changes:

- `WebGLBackdrop` now stores `environmentRotationSource` metadata with mode `source-p1-env-rotation-y-negative-rotationAdjustment-from-demorgen`, `demorgenIndex`, project count, theta degrees, rotation adjustment degrees, and expected environment group `rotation.y`.
- `createWorkScene()` records the `demorgen` index while preserving the existing source-derived `rotationAdjustment = -theta * index` and `environmentGroup.rotation.y = -MathUtils.degToRad(rotationAdjustment)` path.
- `__rogierOutputProbe` now exposes `reflectionState.sceneWrap.sourceChildOrderMode`, `reflectionState.sceneWrap.sourceChildOrder`, `reflectionState.environment.rotationSource`, `reflectionState.environment.rotationMatchesSource`, and the same environment rotation metadata under `uniforms.environment`.
- `scripts/probe-output-color.mjs` now asserts source `sceneWrap` child order, rotation source mode, valid `demorgenIndex`/count, `thetaDegrees`, expected rotation Y, `rotationMatchesSource`, and group rotation Y.
- `scripts/audit-renderer-output.mjs` now checks source `p1` hierarchy anchors and rebuild/probe coverage for the environment rotation guardrail.
...
- `node --check scripts/probe-output-color.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-env-rotation-audit.json`; `sourceManagers.p1EnvironmentHierarchy` source checks, rebuild checks, and rebuild probe checks were all true.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop `scripts/probe-output-color.mjs` passed with no browser failures/exceptions/console messages; runtime reported `sourceChildOrder=["blocksWrap","floor","env"]`, `demorgenIndex=8`, `count=10`, `thetaDegrees=36`, `rotationAdjustmentDegrees=-288`, `expectedRotationY=5.026548245743669`, and `rotationMatchesSource=true`.
- Mobile `scripts/probe-output-color.mjs` passed with the same source child order and rotation metadata, with no browser failures/exceptions/console messages.
- Desktop and mobile `scripts/probe-thumb-spotlight.mjs` passed with source spotlight/thumb guardrails intact, `3/9` in-map samples, nonzero `mapLumaMean`, and mobile `mobileYOffset=0.3`.
- `scripts/probe-project-media.mjs` passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks and no browser failures/exceptions/console messages.

Decision: keep this as a source hierarchy/rotation guardrail, not a visual closeout. Phase 1 remains open because this strengthens only the `p1` floor/environment hierarchy evidence; actual floor/environment distribution parity, spotlight/thumb projection transfer feel, and broader `kA/Lu/I1` transfer/composite interpretation remain unresolved.

### S1-266 Thumb Spotlight Projection Sampling Guardrail

This batch extends the source `T1/w1/SD/p1` thumb/spotlight runtime guardrail. It does not change production rendering, shader formulas, light constants, render-target topology, project data, or transfer/color behavior.

Source/decision evidence:

- Source `SD.init()` assigns `J.workScene.spotLight.map=J.workThumbScene.renderManager.renderTargetComposite.texture`, sets spotlight position `(0,0,3.7)`, target `(0,0,-8)`, and intensity `220`.
- Source `p1.update()` owns the spotlight parallax branch: desktop `camera.y * .175`, mobile `.3 + camera.y * .175`.
- Three r164's spotlight-map path stores `light.map`, updates the spotlight shadow matrix, and multiplies direct light by `spotLightMap` through `vSpotLightCoord`; source does not need explicit `castShadow` to use the map projection path.
- Previous batches source-shaped the thumb target, thumb composite, `w1` wrapping, `SD` map binding, and desktop/mobile spotlight branch, but thumb probe did not yet assert that nonzero-progress map content is projectable through the spotlight matrix.

Runtime and tooling changes:

- `__rogierThumbProbe` now includes the existing `spotlightProjectionProbe()` result.
- `scripts/probe-thumb-spotlight.mjs` now asserts the source `SpotLight.map` path, no-shadow-cast projection mode, Three spotlight-map matrix/chunk attribution, source parallax branch, 3x3 active-bounds sample grid, nonzero in-map sample coverage, and nonzero sampled map luma.
- `scripts/audit-renderer-output.mjs` now checks that the thumb probe consumes and asserts this projection sampling surface.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-thumb-projection-audit.json`; `sourceManagers.homeSpotlightMap.rebuildThumbProjectionSamplingProbe=true`.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- Desktop `scripts/probe-thumb-spotlight.mjs` passed at `THUMB_PROGRESS=0.27` with no browser failures/exceptions/console messages; spotlight projection reported `3/9` in-map samples and nonzero `mapLumaMean`.
- Mobile `scripts/probe-thumb-spotlight.mjs` passed at `THUMB_PROGRESS=0.27` with no browser failures/exceptions/console messages; spotlight projection reported `3/9` in-map samples, nonzero `mapLumaMean`, and `mobileYOffset=0.3`.
- Desktop and mobile `scripts/probe-output-color.mjs` passed with no browser failures/exceptions/console messages.
- `scripts/probe-project-media.mjs` passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks and no browser failures/exceptions/console messages.

Decision: keep this as a source projection-content guardrail, not a visual closeout. Phase 1 remains open because this strengthens evidence for the spotlight/thumb input path only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-265 Rejected Debug Shortcut Retirement

This batch retires previously rejected query-only transfer/color shortcuts from the current runtime and QA scripts. It does not promote a new visual fix, change source formulas, alter production render-target topology, or close any remaining Phase 1 visual residual.

Source/decision evidence:

- Source `SD.init()` assigns `J.workScene.spotLight.map=J.workThumbScene.renderManager.renderTargetComposite.texture`; disabling the map is not source-shaped.
- Source `OA/CA` and renderer evidence supports the current source blend/darken formulas and `srgb` renderer output, not a shader-level `debug-composite-transfer` gamma lift.
- Earlier thumb colorspace attribution showed render-target color-space metadata variants were diagnostic only, not source-backed thumb/spotlight transfer fixes.
- Phase 1 rules explicitly reject `debug-composite-transfer=*`, `debug-spotlight-map=off`, and `debug-thumb-colorspace=*` as implementation paths.

Runtime and tooling changes:

- Removed `debug-composite-transfer` parsing, `debugCompositeTransferMode`, `uDebugTransferMode`, and the gamma/linearize branch from `homeCompositeDebugFragment`.
- Removed `debug-spotlight-map=off`; `homeSpotlightMap()` now always returns `thumbCompositeTarget.texture`.
- Removed `debug-thumb-colorspace=*` target color-space mutation.
- `scripts/probe-output-color.mjs` no longer treats `debug-composite-transfer` as a supported debug composite mode.
- `scripts/compare-home-brightness-attribution.mjs` no longer constructs `spotlight-map-off`, `scene-transfer`, or `spotlight-map-off-darken-off` variants.
- `scripts/compare-composite-stages.mjs` no longer constructs transfer-mode variants or emits transfer-mode summaries.
- `scripts/compare-spotlight-map.mjs` and `scripts/compare-thumb-colorspace.mjs` now capture only the source/default path instead of rejected variants.
- `scripts/audit-renderer-output.mjs` now reads those scripts and exposes `debugShortcuts.noRejectedCompositeTransferDebug`, `noRejectedSpotlightMapOffDebug`, `noRejectedThumbColorspaceDebug`, and `homeSpotlightMapAlwaysSourceThumbComposite`.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/probe-project-media.mjs` passed.
- `node --check scripts/compare-composite-stages.mjs` passed.
- `node --check scripts/compare-home-brightness-attribution.mjs` passed.
- `node --check scripts/compare-spotlight-map.mjs` passed.
- `node --check scripts/compare-thumb-colorspace.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-debug-shortcuts-audit.json`; all four `debugShortcuts` checks were true.
- Desktop and mobile `scripts/probe-output-color.mjs` passed with no browser failures/exceptions/console messages.
- `scripts/probe-thumb-spotlight.mjs` passed with source `thumbCompositeTarget.texture` map present, empty map color space, and `900x900` thumb/composite targets.
- `scripts/probe-project-media.mjs` passed with `gc-2026` and `hashgraph-vc` retaining `5/5` visible media tracks and no browser failures/exceptions/console messages.

Decision: keep rejected shortcuts retired. Reintroduce them only if new mirrored-source evidence shows an equivalent source path. Phase 1 remains open because this is source-discipline/guardrail cleanup only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-264 `C1/Se/I1` Fluid Strength Constructor Ownership

This batch aligns source `C1.uFluidStrength` constructor/runtime ownership. It does not change shader formulas, visual constants, render-target topology, spotlight/thumb projection wiring, project media formulas, or route data.

Source evidence:

- Source `C1` constructs `uFluidStrength:new I(.5)` on the `A1/C1` material surface.
- Source `Se.init()` initializes `this.settings.fluidStrength=0`.
- Source `Se.setFluidStrength(e,t=.5)` is the later runtime writer to `J.mainScene.renderManager.compositeMaterial.uniforms.uFluidStrength.value` from `this.settings.fluidStrength`.
- Source `I1.update()` gates `this.fluidSimulation.update()` from `this.compositeMaterial.uniforms.uFluidStrength.value>0`, then binds `this.fluidSimulation.fbos.main.texture` when the fluid branch is enabled.

Runtime and tooling changes:

- `createPreCompositeMaterial()` now constructs `uFluidStrength` with `SOURCE_C1_FLUID_STRENGTH_DEFAULT=0.5`.
- The `C1/A1` material records `sourceFluidStrengthConstructorDefault`, `sourceFluidStrengthRuntimeOwnership=source-Se-setFluidStrength-writes-C1-uFluidStrength`, and `sourceFluidStrengthStateDivergenceMode=source-C1-constructor-0_5-Se-settings-initial-0` on `userData`.
- Home and WorkGallery entry no longer call compensation `setFluidStrength(.5)` setters solely to recover the source constructor default.
- The frame loop no longer rewrites `C1.uFluidStrength` from `settingsState.fluidStrength`; `setFluidStrength()` remains the runtime writer after construction.
- The main fluid update gate now reads the current `C1.uFluidStrength` uniform, not `settingsState.fluidStrength`.
- Output probes expose `settingsState.fluidStrength=0`, `uFluidStrength=0.5`, `fluidStrengthUniformMatchesState=false`, constructor default, runtime ownership, and divergence mode.
- `scripts/probe-output-color.mjs` asserts the constructor/default divergence on Home desktop and mobile.
- `scripts/audit-renderer-output.mjs` checks source `C1` and `Se` anchors, rebuild constructor/default markers, no tick rewrite, no Home/Gallery compensation setter, and C1-uniform fluid update gate.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-c1-fluid-strength-audit.json`.
- Desktop output probe passed with `settingsState.fluidStrength=0`, `C1.uFluidStrength=0.5`, and `fluidStrengthUniformMatchesState=false`: `/tmp/rd-c1-fluid-strength-output-desktop`.
- Mobile output probe passed with the same constructor divergence markers: `/tmp/rd-c1-fluid-strength-output-mobile`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages with no failures/exceptions/console messages: `/tmp/rd-c1-fluid-strength-media`.

Decision: keep `C1.uFluidStrength` constructor-owned at `.5` until `Se.setFluidStrength()` writes it. Do not force initial parity with `settingsState.fluidStrength=0`; that divergence is source-shaped. Phase 1 remains open because this closes one constructor/runtime ownership mismatch only; actual spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-263 `T1/w1/SD/p1` Mobile Thumb Spotlight Guardrail

This batch expands the thumb/spotlight projection guardrail to cover the source mobile branch. It does not change production shader formulas, render targets, spotlight constants, thumb content, visual constants, or route behavior.

Source evidence:

- Source `SD.init()` assigns `J.workScene.spotLight.map=J.workThumbScene.renderManager.renderTargetComposite.texture`.
- Source `SD.init()` sets `J.workScene.spotLight.position.set(0,0,3.7)`, `target.position.set(0,0,-8)`, and `intensity=220`.
- Source `p1.update()` sets `this.spotLight.position.x=this.camera.position.x*.175`.
- Source `p1.update()` uses a viewport branch for spotlight Y: desktop `this.spotLight.position.y=this.camera.position.y*.175`, mobile `this.spotLight.position.y=.3+this.camera.position.y*.175`.
- Source `T1.resize(e,t,n)` calls `this.renderManager.resize(t,t,1)`, so the thumb render-manager targets are square CSS-height targets with DPR `1`.

Runtime and tooling changes:

- `scripts/probe-thumb-spotlight.mjs` now accepts `VIEWPORT=desktop|mobile` and sets the CDP viewport accordingly.
- The probe derives expected `parallaxYOffsetMode`, `mobileYOffset`, spotlight position, and thumb target size from the viewport instead of hard-coding desktop values.
- Probe summaries now include `viewportName` and the viewport dimensions.
- `scripts/audit-renderer-output.mjs` now reads the thumb probe script and checks source mobile parallax evidence plus rebuild/probe coverage for both desktop and mobile source branches.

Verification:

- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `node --check scripts/audit-renderer-output.mjs` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-thumb-mobile-guard-audit.json`; `sourceMobileSpotlightParallaxBranch=true` and `rebuildMobileSpotlightParallaxProbe=true`.
- Desktop thumb spotlight probe passed with `VIEWPORT=desktop`, `parallaxYOffsetMode=source-p1-desktop-camera-y-parallax`, spotlight position `[0,0,3.7]`, and `900x900` thumb/composite targets: `/tmp/rd-thumb-mobile-guard-desktop`.
- Mobile thumb spotlight probe passed with `VIEWPORT=mobile`, `parallaxYOffsetMode=source-p1-mobile-0_3-plus-camera-y-parallax`, `mobileYOffset=0.3`, spotlight position `[0,0.3,3.7]`, and `844x844` thumb/composite targets: `/tmp/rd-thumb-mobile-guard-mobile`.
- Both browser probes reported no failed requests, runtime exceptions, or filtered console errors.

Decision: keep the source mobile spotlight/thumb guardrail and run both desktop and mobile thumb probes when touching `SD/p1/T1/w1` projection, spotlight map, or thumb target code. Phase 1 remains open because this is QA/evidence hardening only; actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-262 `VA/GA` uUvOffset Shader Declaration

This batch aligns the ordinary work `uUvOffset` shader declaration while preserving the source runtime uniform shape. It does not change mouse-plane sizing formulas, local mouse simulation targets, shader math, visual constants, render targets, spotlight/thumb map wiring, or auxiliary block material strategy.

Source evidence:

- Source `HA` declares `uniform vec3 uUvOffset;` before `uUvOffsetScale`.
- Source `HA` computes `vec2 mouseUv = newUv + uUvOffset.xy` and then `mouseUv /= uUvOffsetScale`; the shader reads only the `xy` components.
- Source `VA.customUniforms` constructs `uUvOffset:new I(new Q)` and `uUvOffsetScale:new I(1)`, so the runtime uniform value is a `Vector2`.
- Source `GA.createPlane()` writes `this.material.customUniforms.uUvOffset.value.x=...`, `.y=...`, and `this.material.customUniforms.uUvOffsetScale.value=t`; it does not write a `z` component.

Runtime and tooling changes:

- `workBlockSourceHaVertexShader` now declares `uniform vec3 uUvOffset;`.
- Shared `workBlockVertexPars` now declares `uniform vec3 uUvOffset;` for the ordinary-work shader surface that mirrors `HA`.
- The runtime uniform remains `uUvOffset: { value: sourceMouseUvOffset() }`, matching the source `Vector2`/XY write ownership.
- `__rogierOutputProbe.mouseSimulation.active` exposes `uvOffsetShaderDeclaration=source-HA-vec3-uUvOffset`, `uvOffsetRuntimeBridgeMode=source-VA-uniform-value-Vector2-GA-writes-xy-shader-reads-xy`, and source-shape booleans for both the shader declaration and runtime bridge.
- `scripts/probe-output-color.mjs` asserts both active mouse probe markers.
- `scripts/dump-va-shader.mjs` now expects the source `vec3` declaration and records runtime/source evidence for source `Vector2` plus `GA` XY writes.
- `scripts/audit-renderer-output.mjs` checks the source shader declaration, source runtime `Vector2`/XY writes, rebuild work/shared shader declarations, and probe marker coverage.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-uvoffset-vec3-audit.json`; `sourceShaderVec3`, `sourceRuntimeVector2`, `rebuildWorkShaderVec3`, `rebuildAuxShaderVec3`, and `rebuildRuntimeVector2` are all `true`.
- Shader dump passed and wrote `/tmp/rd-uvoffset-vec3-shader`; `VA-work` vertex and fragment deltas are `0`, and `sourceUvOffsetBeforeRevealUniforms` matches source/rebuild.
- Desktop output probe passed with `uvOffsetShaderDeclaration=source-HA-vec3-uUvOffset`, `uvOffsetRuntimeBridgeMode=source-VA-uniform-value-Vector2-GA-writes-xy-shader-reads-xy`, and matching source-shape booleans: `/tmp/rd-uvoffset-vec3-output-desktop`.
- Mobile output probe passed with the same `uUvOffset` shader/runtime markers and source-shape booleans: `/tmp/rd-uvoffset-vec3-output-mobile`.
- Thumb spotlight probe passed and retained the existing spotlight/thumb guardrails: `/tmp/rd-uvoffset-vec3-thumb`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-uvoffset-vec3-media`.

Decision: keep ordinary work shader text on source `uniform vec3 uUvOffset;` while preserving the runtime `Vector2`/XY write bridge documented by source `VA` and `GA`. Do not coerce runtime `uUvOffset` to `Vector3`, or revert the source shader declaration to `vec2`, without mirrored-bundle evidence. Phase 1 remains open because this closes a shader declaration/runtime ownership mismatch only; actual spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-260 `o1` Floor Material Uniform Order

This batch aligns one floor material constructor-surface mismatch. It does not change shader formulas, render targets, floor constants, normal-map binding, reflector blur behavior, or any visual tuning knobs.

Source evidence:

- Source `o1` constructs base uniforms as `tReflect`, `uMapTransform`, `uMatrix`, `uColor`, `uReflectivity`, `uMirror`, `uFloorMixStrength`, and `uNormalDistortionStrength`.
- Source `o1` then appends `tNormalMap` and `uNormalScale` in the `normalMap` branch.
- Source reassigns the existing `uMapTransform` slot from the normal texture matrix when no diffuse map is supplied; that does not move the key's object-order position.
- The rebuild had all active floor uniforms and source values, but placed `tNormalMap` before `uReflectivity`, `uMirror`, `uFloorMixStrength`, and `uNormalDistortionStrength`.

Runtime and tooling changes:

- `createFloorMaterial()` now declares floor uniforms in source order.
- `SOURCE_O1_FLOOR_UNIFORM_KEYS` records the expected source order.
- `__rogierOutputProbe.uniforms.floor` exposes `sourceUniformKeys`, runtime `uniformKeys`, and `matchesSourceOrder`.
- `scripts/probe-output-color.mjs` asserts the exact source order.
- `scripts/audit-renderer-output.mjs` checks the source `o1` order, rebuild factory order, and output-probe coverage.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-floor-uniform-order-audit.json`; `sourceUniformOrder`, `rebuildUniformOrder`, and `rebuildProbeOrder` are all `true`.
- Desktop output probe passed with floor `uniformKeys` matching `SOURCE_O1_FLOOR_UNIFORM_KEYS` and `matchesSourceOrder=true`: `/tmp/rd-floor-uniform-output-desktop`.
- Mobile output probe passed with the same floor uniform order and `matchesSourceOrder=true`: `/tmp/rd-floor-uniform-output-mobile`.
- Thumb spotlight probe passed and retained thumb material order plus spotlight-map guardrails: `/tmp/rd-floor-uniform-thumb`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-floor-uniform-media`.

Decision: keep `o1` floor material uniforms in source order. Do not move `tNormalMap` before the reflection scalar uniforms without mirrored-bundle evidence. Phase 1 remains open because this closes one floor constructor-surface mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-259 `lA` Main Composite Bool Constructor Ownership

This batch aligned source `lA` bool uniform constructor ownership for the `lA/aA` main-composite source surface. It does not change the active default `I1/C1` screen path, shader formulas, render-manager pass order, render targets, visual constants, or debug transfer modes.

Source evidence:

- Source `lA` constructs `uniforms:{tScene:new I(null),tBloom:new I(null),tBlur:new I(null),tFluid:new I(null),tMouseSim:new I(null),boolBloom:new I(!1),boolFluid:new I(!1),boolLuminosity:new I(!1),boolFxaa:new I(!1)}`.
- Source `Lu.update()` later writes `this.compositeMaterial.uniforms.boolBloom.value=this.settings.bloom.enabled`, `boolFluid`, `boolLuminosity`, and `boolFxaa` from settings immediately before rendering that composite screen.
- Source `I1` uses `C1/A1` as its active default composite material; this batch is about the retained `lA/aA` source-surface material/probe artifact, not a visual closeout for the active `I1/C1` chain.

Runtime and tooling changes:

- `createMainCompositeMaterial()` now constructs `boolBloom`, `boolFluid`, `boolLuminosity`, and `boolFxaa` as `false`.
- The material records `sourceConstructorBoolDefaults` and `sourceRuntimeBoolOwnership=source-Lu-update-writes-lA-bools-from-settings-before-composite-render` in `userData`.
- The rebuild no longer copies `sourceMainRenderSettings` bool values into the `lA` constructor; current `lA` bool values remain the constructor defaults because this source-surface material is not the active default `I1/C1` screen material.
- `__rogierOutputProbe.uniforms.mainComposite` exposes constructor bool defaults, runtime ownership, and current bool values.
- `scripts/probe-output-color.mjs` asserts constructor-false defaults, source runtime ownership, and current false bool values for `mainComposite`.
- `scripts/audit-renderer-output.mjs` checks source `lA` constructor defaults, source `Lu.update()` runtime bool writes, rebuild constructor defaults, old settings-in-constructor absence, and probe coverage.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-la-bool-audit.json`; `sourceBoolConstructorDefaults=true`, `sourceLuRuntimeBoolWrites=true`, `rebuildBoolConstructorDefaults=true`, and `rebuildProbeCoverage=true`.
- Desktop output probe passed with no failures/exceptions/console messages: `/tmp/rd-la-bool-output-desktop`.
- Mobile output probe passed with no failures/exceptions/console messages: `/tmp/rd-la-bool-output-mobile`.
- Thumb spotlight probe passed with the existing M1/E1 constructor/order and spotlight-map guardrails: `/tmp/rd-la-bool-thumb`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-la-bool-media`.

Decision: keep `lA` bool uniforms constructor-false and keep runtime bool ownership documented as source `Lu.update()`, without copying `sourceMainRenderSettings` into the constructor. Do not treat this as active default main-screen visual parity, because source `I1` uses `C1/A1` there. Phase 1 remains open because this closes one retained source-surface constructor mismatch only; spotlight/thumb projection transfer feel, broader active `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-258 `sg` Luminosity Constructor Settings Ownership

This batch aligned source `sg` luminosity helper-material constructor/settings ownership. It does not change the luminosity shader formula, render-manager pass order, render targets, thresholds, smoothing values, visual constants, or debug transfer modes.

Source evidence:

- Source `sg` constructs `uniforms:{tMap:new I(null),uThreshold:new I(1),uSmoothing:new I(1)}` with raw GLSL3/no-blending/depth-disabled material state.
- Source `Lu.initRenderer()` creates `this.luminosityMaterial=new sg`, then writes `this.luminosityMaterial.uniforms.uThreshold.value=this.settings.luminosity.threshold` and `uSmoothing` from work settings.
- Source `I1.initRenderer()` follows the same constructor-then-settings-write ownership for the main render manager.

Runtime and tooling changes:

- `createLuminosityMaterial()` now constructs `tMap:null`, `uThreshold=1`, and `uSmoothing=1`.
- The material records `sourceConstructorThreshold`, `sourceConstructorSmoothing`, and `sourceInitSettingsOwnership=source-Lu-I1-initRenderer-writes-sg-threshold-smoothing-after-construction` in `userData`.
- The rebuild then writes final `uThreshold` and `uSmoothing` from the passed render-manager settings after construction.
- `__rogierOutputProbe.uniforms.passMaterials` exposes constructor defaults, init-settings ownership, and final threshold/smoothing for `luminosity`, `workLuminosity`, and `mainLuminosity`.
- `scripts/probe-output-color.mjs` asserts constructor defaults of `1`, source `Lu/I1.initRenderer()` settings ownership, and final settings parity: work luminosity `0.1/0.95`, main luminosity `0.1/1`.
- `scripts/audit-renderer-output.mjs` checks source `sg` constructor defaults, source `Lu/I1.initRenderer()` settings writes, rebuild constructor defaults followed by settings writes, and runtime probe coverage.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-sg-luminosity-audit.json`; `sourceConstructorDefaults=true`, `sourceLuInitSettingsWrites=true`, `sourceI1InitSettingsWrites=true`, `rebuildConstructorDefaultsThenSettings=true`, and `rebuildProbeCoverage=true`.
- Desktop output probe passed with no failures/exceptions/console messages: `/tmp/rd-sg-luminosity-output-desktop`.
- Mobile output probe passed with no failures/exceptions/console messages: `/tmp/rd-sg-luminosity-output-mobile`.
- Thumb spotlight probe passed with the existing M1/E1 constructor/order and spotlight-map guardrails: `/tmp/rd-sg-luminosity-thumb`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-sg-luminosity-media`.

Decision: keep `sg` luminosity materials constructed with source defaults `tMap:null`, `uThreshold=1`, and `uSmoothing=1`, then write final threshold/smoothing through source `Lu/I1.initRenderer()`-style settings ownership after construction. Do not restore constructor-time settings values without mirrored-bundle evidence. Phase 1 remains open because this closes one helper-material constructor/settings ownership mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-257 `OA/Lu` Work Composite Bool Constructor Ownership

This batch aligned source `OA` bool uniform constructor/runtime ownership. It does not change shader formulas, work render settings, pass order, render targets, visual constants, or debug transfer modes.

Source evidence:

- Source `OA` constructs `uniforms:{...,boolBloom:new I(!1),boolFluid:new I(!1),boolLuminosity:new I(!1),boolFxaa:new I(!1),...}`.
- Source `kA.initSettings()` enables work `luminosity` and `bloom`, but those settings are not written into the `OA` constructor.
- Source `Lu.update()` writes `this.compositeMaterial.uniforms.boolBloom.value=this.settings.bloom.enabled`, `boolFluid`, `boolLuminosity`, and `boolFxaa` from `this.settings` immediately before assigning `this.screen.material=this.compositeMaterial` and rendering the work composite target.

Runtime and tooling changes:

- `createCompositeMaterial()` now constructs `boolBloom`, `boolFluid`, `boolLuminosity`, and `boolFxaa` as `false`.
- The material records `sourceConstructorBoolDefaults` and `sourceRuntimeBoolOwnership` in `userData`.
- `__rogierOutputProbe.uniforms.composite` exposes constructor bool defaults, runtime ownership, runtime bool values, and runtime/settings parity.
- `scripts/probe-output-color.mjs` asserts constructor-false defaults and runtime bools matching source work settings: `boolBloom=true`, `boolFluid=false`, `boolLuminosity=true`, `boolFxaa=false`.
- `scripts/audit-renderer-output.mjs` checks source/rebuild `OA` constructor defaults and source/rebuild `Lu.update()` runtime bool writes.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-oa-bool-audit.json`; `sourceBoolConstructorDefaults=true`, `sourceRuntimeBoolWrites=true`, `rebuildBoolConstructorDefaults=true`, `rebuildRuntimeBoolWrites=true`, and recursive false/null review stayed at the expected `19` source-negative/default entries.
- Desktop output probe passed on the `PROBE_WAIT=45000` rerun with no failures/exceptions/console messages: `/tmp/rd-oa-bool-output-desktop-rerun`.
- Mobile output probe passed on the `PROBE_WAIT=45000` rerun with no failures/exceptions/console messages: `/tmp/rd-oa-bool-output-mobile-rerun`.
- Thumb spotlight probe passed with the existing M1/E1 constructor/order and spotlight-map guardrails: `/tmp/rd-oa-bool-thumb`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-oa-bool-media`.

Decision: keep `OA` work composite bool uniforms constructor-false and owned by source `Lu.update()` runtime writes before the composite render. Do not restore constructor-time `settings.bloom.enabled` / `settings.luminosity.enabled` values without mirrored-bundle evidence. Phase 1 remains open because this closes one constructor/runtime ownership mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-256 `M1` Thumb Material Uniform Declaration Order

This batch aligned the source `M1` thumb material uniform declaration order. It does not change thumb shader formulas, texture binding behavior, render targets, spotlight projection constants, route data, or visual tuning.

Source evidence:

- Source `M1` constructs `uniforms:{tMap:new I(null),uResolution:new I(new Q),uMapSize:new I(new Q),uProgress:new I(1),uTransitionCount:new I(150),uTransitionSmoothness:new I(.2)}`.
- Source `E1.setImage(e)` still writes `uMapSize` and `uResolution` after texture binding; this batch only covers constructor declaration order.

Runtime and tooling changes:

- `createThumbPlane()` now declares `uResolution` before `uMapSize`, matching source `M1`.
- Thumb material `userData` records `Object.keys(material.uniforms)` after construction.
- `__rogierThumbProbe.thumbMaterial.uniformOrder` exposes the runtime uniform order.
- `scripts/probe-thumb-spotlight.mjs` asserts `["tMap","uResolution","uMapSize","uProgress","uTransitionCount","uTransitionSmoothness"]`.
- `scripts/audit-renderer-output.mjs` adds `orderedIncludes(...)` and checks source/rebuild `M1` uniform order alongside the existing constructor-null and post-`setImage` binding checks.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-thumb-m1-order-audit.json`; `sourceM1UniformOrder=true`, `rebuildM1UniformOrder=true`, and recursive false/null review stayed at the expected `19` source-negative/default entries.
- Thumb spotlight probe passed with `uniformOrder=["tMap","uResolution","uMapSize","uProgress","uTransitionCount","uTransitionSmoothness"]`: `/tmp/rd-thumb-m1-order-probe`.
- Desktop output probe rerun passed with no failures/exceptions/console messages: `/tmp/rd-thumb-m1-order-output-desktop-rerun`.
- Mobile output probe passed with no failures/exceptions/console messages: `/tmp/rd-thumb-m1-order-output-mobile`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-thumb-m1-order-media`.

Decision: keep `M1` thumb material uniforms declared in source order. Do not move `uMapSize` before `uResolution` without mirrored-bundle evidence. Phase 1 remains open because this closes one constructor-surface ordering mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-255 `M1/E1` Thumb Material Constructor and Image Binding Ownership

This batch aligned the source thumb material image-default lifecycle. It does not change thumb shader formulas, spotlight projection constants, render targets, route data, or visual tuning.

Source evidence:

- Source `M1` constructs `tMap:new I(null)`.
- Source `M1` constructs `uResolution:new I(new Q)` and `uMapSize:new I(new Q)`, i.e. zero-size vectors.
- Source `E1.constructor(e)` creates `new M1`, calls `this.setImage(e)`, creates the mesh, and scales it to `2,2,2`.
- Source `E1.setImage(e)` awaits `Xt.thumbsReady`, awaits `Xt.getProjectThumbById(e).src`, assigns the texture to `this.material.uniforms.tMap.value`, then writes both `uMapSize` and `uResolution` to `1,1`.
- Source does not construct `M1` with a placeholder texture or constructor-time `1,1` vectors.

Runtime and tooling changes:

- `createThumbPlane()` now constructs `tMap` with `null`.
- `createThumbPlane()` now constructs `uMapSize` and `uResolution` with empty `Vector2()` values.
- Thumb material `userData` records the source constructor mode, initial null map state, and initial zero vector sizes.
- `setSourceThumbImage()` remains the source post-`thumbsReady` binding point for texture plus `1x1` map/resolution writes.
- `__rogierThumbProbe.thumbMaterial` exposes constructor mode, constructor null/zero defaults, set-image binding mode, final map binding, final map size, and final resolution.
- `scripts/probe-thumb-spotlight.mjs` asserts constructor-null/zero defaults and final texture/`1x1` binding.
- `scripts/audit-renderer-output.mjs` checks source `M1/E1` anchors, rebuild source-null constructor defaults, and rejects the old placeholder/constructor `1,1` path inside `createThumbPlane()`.
...
- `node --check scripts/probe-project-media.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-thumb-m1-audit.json`; `sourceManagers.thumbE1.ownership.sourceM1ConstructorDefaults=true`, `rebuildM1ConstructorDefaults=true`, and recursive false/null review stayed at the expected `19` source-negative/default entries.
- Thumb spotlight probe passed with `constructorMode=source-M1-constructor-null-tMap-zero-size-vectors`, `constructorTMapWasNull=true`, constructor vectors `[0,0]`, `mapBound=true`, and final `mapSize` / `resolution` `[1,1]`: `/tmp/rd-thumb-m1-probe`.
- Desktop output probe passed with no failures/exceptions/console messages: `/tmp/rd-thumb-m1-output-desktop`.
- Mobile output probe passed with no failures/exceptions/console messages: `/tmp/rd-thumb-m1-output-mobile`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-thumb-m1-media`.

Decision: keep thumb material constructor defaults source-null/zero-size and leave texture plus `1x1` sizing to the source `E1.setImage()` lifecycle. Do not restore placeholder texture or constructor-time `1,1` vectors for `M1` without mirrored-bundle evidence. Phase 1 remains open because this closes one thumb lifecycle ownership mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-254 `Se` Main/Media Background Color State Tween Ownership

This batch aligned the remaining source `Se.settings` color-state ownership slice. It does not change visual constants, shader formulas, render targets, spotlight/thumb map wiring, or route data.

Source evidence:

- Source `Se.init()` initializes `this.settings.mainColor={r:0,g:0,b:0}` and `this.settings.media.background={r:0,g:0,b:0}`.
- Source `Se.setMainColor(e,t)` parses through `formatColor(e)`, queries `.c-color`, and when `t===0` writes `this.settings.mainColor` before setting DOM color from that state.
- Source `Se.setMainColor(e,t)` otherwise tweens `this.settings.mainColor` and writes every `.c-color` DOM style from `this.settings.mainColor` in `onUpdate`, using source `Fn` rounding.
- Source `Se.setMediaBackground(e,t)` writes duration-zero state to `this.settings.media.background` and `J.mediaScene.mediaItems.backgroundColor`.
- Source `Se.setMediaBackground(e,t)` otherwise tweens `this.settings.media.background` and writes `J.mediaScene.mediaItems.backgroundColor` from state in `onUpdate`.
- Source does not create one GSAP tween per `.c-color` element and does not keep standalone media background state outside `this.settings.media.background`.

Runtime and tooling changes:

- `settingsState` now includes `mainColor` and `media.background` with source initial values.
- `setMainColor()` now writes `.c-color` DOM color from `settingsState.mainColor` for both duration-zero and tweened paths.
- `setMainColor()` uses source `Fn`-style rounding through `sourceRounded()` / `sourceMainColorCss()`: duration-zero uses two decimals and tween `onUpdate` uses zero decimals.
- `setMediaBackground()` now updates `mediaBackground` and each media plane `uBackgroundColor` from `settingsState.media.background`.
- Rebuild-only `mainColorTweens`, `mediaBackgroundTweens`, and `mediaBackgroundState` are removed.
- `__rogierOutputProbe.settings.work.settingsStateOwnership` exposes `mainColor`, `mediaBackground`, `mainColorElementsMatchState`, `mediaBackgroundMatchesState`, and `mediaPlaneBackgroundsMatchState`.
- `scripts/probe-output-color.mjs` asserts the new state/fan-out parity fields.
- `scripts/audit-renderer-output.mjs` checks source `Se.setMainColor()` / `setMediaBackground()` anchors, rebuild state-owned paths, and rejects the old direct DOM tween / standalone media state paths.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-color-state-audit-final.json`; `sourceManagers.Se.colorStateOwnership.source=true`, `rebuild=true`, and recursive false/null review stayed at the expected `19` source-negative/default entries.
- Desktop output probe passed with `mainColorElementsMatchState=true`, `mediaBackgroundMatchesState=true`, and `mediaPlaneBackgroundsMatchState=true`: `/tmp/rd-color-state-output-desktop-final`.
- Mobile output probe passed with the same color state/fan-out parity markers: `/tmp/rd-color-state-output-mobile-final`.
- Thumb spotlight probe passed and retained the source thumb/light state guardrails: `/tmp/rd-color-state-thumb-final`.
- Project media probe passed for `/gc-2026/` and `/hashgraph-vc/`, retaining five visible media tracks on both pages: `/tmp/rd-color-state-media-final`.

Decision: keep main color and media background color owned by source-style `Se.settings` state and fan out DOM/media uniforms from state updates. Do not reintroduce per-element main-color tweens or standalone media-background state without mirrored-bundle evidence. Phase 1 remains open because this closes state tween ownership only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.

### S1-253 `VA/XA` Work Material uCoords Runtime Ownership

This batch aligned source `VA` and `XA` material `uCoords` ownership. It does not change shader formulas, visual constants, render-target sizes, spotlight/thumb map wiring, or route data.

Source evidence:

- Source `VA.update(e,t,n,i)` writes `this.customUniforms.uTime.value=e` and `this.customUniforms.uCoords.value.set(Pe.w*i,Pe.h*i)`.
- Source `XA.update(e,t,n,i)` uses the same `this.customUniforms.uCoords.value.set(Pe.w*i,Pe.h*i)` path for the about auxiliary material.
- Source `p1.resize(e,t,n)` calls `super.resize(e,t,Math.min(n,1.5))`, so the work update/resize DPR argument is capped at `1.5`.
- Source `Pe.w` and `Pe.h` are CSS viewport dimensions, so the material `uCoords` value is viewport width/height times capped work DPR, not rounded render-target dimensions.
- Source `KA.update()` only updates `uTime`, so the floating auxiliary material is intentionally not moved to this runtime `uCoords` path.

Runtime and tooling changes:

- `sourceWorkViewportCoords()` now returns `window.innerWidth/window.innerHeight * sourceWorkDpr()`.
- `setSourceWorkMaterialUCoords()` applies that source value to ordinary work materials.
- `resize()` and `updateVisibleWorkItems()` now use source viewport coords for ordinary work items instead of rounded `workRenderWidth/workRenderHeight` or `workRawTarget.width/height`.
- `resizeAuxiliaryBlocks()` and `updateAuxiliaryBlocks()` apply the same source coords to the about `XA` auxiliary material; floating `KA` keeps the existing resize-owned coords.
- `__rogierOutputProbe.p1UpdateCulling` exposes `sourceUCoordsMode`, `sourceUCoordsExpected`, and `uCoordsMatchesSource`, while keeping `uCoordsMatchesWorkTarget` as a diagnostic.
- `scripts/probe-output-color.mjs` asserts source mode and source-coordinate parity instead of rounded work-target parity.
- `scripts/audit-renderer-output.mjs` extracts source `VA` and `XA`, checks rebuild ownership markers, and rejects the old rounded work-target `uCoords` writes.
...
- `node --check scripts/probe-thumb-spotlight.mjs` passed.
- `ASTRO_TELEMETRY_DISABLED=1 npm run build` passed.
- `node scripts/audit-renderer-output.mjs` passed and wrote `/tmp/rd-ucoords-audit-final.json`; recursive false/null review stayed at the expected `19` source-negative/default entries.
- Desktop output probe passed with `sourceUCoordsExpected=[1440,900]`, `uCoords=[1440,900]`, `uCoordsMatchesSource=true`, and `uCoordsMatchesWorkTarget=true`: `/tmp/rd-ucoords-output-desktop`.
- Mobile DPR `1.25` output probe passed with render/work target `488x1055`, `sourceUCoordsExpected=[487.5,1055]`, `uCoords=[487.5,1055]`, `uCoordsMatchesSource=true`, and `uCoordsMatchesWorkTarget=false`: `/tmp/rd-ucoords-output-mobile-dpr125`.
- Thumb spotlight probe passed and retained the source thumb/light state guardrails: `/tmp/rd-ucoords-thumb`.

Decision: keep ordinary work and about `XA` material `uCoords` sourced from CSS viewport dimensions times capped work DPR, not rounded render-target dimensions. Do not apply the same update ownership to floating `KA` without mirrored-bundle evidence. Phase 1 remains open because this closes one runtime ownership mismatch only; spotlight/thumb projection transfer feel, broader `kA/Lu/I1` transfer/composite interpretation, and floor/environment distribution gaps remain unresolved.
