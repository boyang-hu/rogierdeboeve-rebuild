...
- `VA/GA` still use a stable Three r164 shader bridge, not a byte-for-byte source `HA/zA` replacement, though the current generated `VA-work` vertex dump no longer has rebuild-only r164 include residuals and the fragment dump no longer has live include/uniform residuals.
...
Current next batch: continue Phase 1 Home WebGL. Prioritize source-backed work in this order: remaining `VA/GA` output and spotlight/thumb projection evidence, then unresolved `A1/OA/kA/Lu` transfer/target graph evidence, while keeping project pages as regression gates. Do not rank next work by visual gain alone. Phase 2 should not start yet.
