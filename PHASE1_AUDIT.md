...
- Ordinary `VA-work` now uses direct source-shaped `HA/zA` templates, but it is still not byte-for-byte source-isomorphic because the documented technical bridges remain: `uUvOffset` stays `vec2` for the source runtime `Vector2`, `SPECULAR` stays bridged to Three r164 `USE_SPECULAR`, and local Three physical-light chunks still provide compiled lighting. Auxiliary block shaders remain on the safer r164 bridge because they are not ordinary source `VA-work`.
- `A1/OA/kA/Lu` are source-shaped in pieces and now have source-shaped `nD/Iu/Lu/I1/Lo` update sequencing, but they are still not a byte-for-byte source-isomorphic render-manager graph.
...
- Prioritize source-structure, source-value, and source-behavior parity over any visual-payoff scoring. Visual improvement is not an implementation goal by itself; production changes need mirrored-bundle evidence even when they look better.

Current next batch: continue Phase 1 Home WebGL. Prioritize source-backed work in this order: review whether the remaining `SPECULAR`/`USE_SPECULAR` bridge can only be closed by source-porting the physical-light chunk, then continue spotlight/thumb projection evidence, then unresolved `A1/OA/kA/Lu` transfer/target graph evidence, while keeping project pages as regression gates. Do not rank next work by visual gain alone. Phase 2 should not start yet.
...
| --- | --- | --- | --- | --- | --- | --- |
| 127 | S1-200 | `zA` direct work fragment shader template | Source `zA` is a complete ordinary work-block fragment shader surface, not a Three r164 `meshphysical` fragment template with source-absent paths patched and stripped after generation. After S1-197/S1-199 the rebuild still generated the ordinary `VA-work` fragment through the r164 template and then removed paths with strip comments, leaving avoidable generated-surface drift even though include/uniform anchors matched. Source `zA` defines the physical material surface with the older `SPECULAR` macro, while local Three r164's `lights_physical_fragment` requires `USE_SPECULAR`, so that compile bridge still has to remain unless the whole physical-light chunk is source-ported and verified. | The ordinary work `VA` variant now assigns a direct source-shaped `zA` fragment shader template instead of patching and stripping the Three r164 fragment template. The source custom declarations, material surface order, fragment include order, random grid alpha tail, mouse-simulation sampling, reveal/vignette alpha logic, and final source tail are preserved; the only intentional source-text bridge left in the work fragment is `SPECULAR -> USE_SPECULAR` for the local Three r164 lighting chunk. Auxiliary block fragments keep the safer r164 bridge because they are not ordinary source `VA-work`. Shader dump now asserts `noR164StripCommentsInWorkFragment`; `VA-work` fragment text delta narrowed to `-1`, with residuals limited to the compile bridge plus whitespace normalization. `git diff --check`, `npm run build`, renderer audit, desktop output probe, mobile output probe, shader dump, thumb spotlight probe, project-media probe, full capture, and band analysis passed. Project media retained five visible tracks on both `gc-2026` and `hashgraph-vc`. Final band deltas were desktop center `+0.0044` and mobile center `+0.0298`, recorded only as regression evidence. | Medium | Keep as source `zA` work-fragment template parity while preserving the documented Three r164 `USE_SPECULAR` technical bridge. Phase 1 remains open for deciding whether that bridge requires a full source physical-light port, plus source-backed spotlight/thumb projection, `OA/CA/kA/Lu` transfer evidence, and mobile/fog-bed residuals. |
...
| `GA/VA` ordinary blocks | Stable bridge, still not full source shader | Material defaults, alpha tail, physical-response probes, vertex world/UV probes, projection coverage, current `VA-work` include/uniform surface, and the r164 `USE_SPECULAR` compile bridge are attributed; full `HA/zA` replacement was rejected as unsafe in the current bridge. | Yes for strict 1:1 | Keep stable bridge for runtime safety only where source-porting is technically blocked; do not mark it accepted without narrower source-safe shader evidence or a documented unavoidable Three bridge. |
