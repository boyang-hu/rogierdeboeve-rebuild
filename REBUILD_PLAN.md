...
| `current batch` | Home WebGL | Batched source `p1/GA` scene-update alignment: restored source `Fog("grey", 0, 100)`, loaded the `/images/cubemaps/01` environment map into the home scene like `p1.addEnvironment()`, and moved per-frame work-block uniforms/mouse textures behind the source-style visibility branch so hidden blocks do not keep running `GA.update()` work. |
