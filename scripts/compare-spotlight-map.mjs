...
const outDir = process.env.OUT_DIR || path.join(tmpdir(), "rogier-spotlight-map");
const port = Number(process.env.CDP_PORT || 9232);
const rebuildUrl = process.env.REBUILD_URL || "http://127.0.0.1:5173";
const waitAfter = Number(process.env.CAPTURE_WAIT || 4200);
...
const chrome = spawn(chromePath, [
  ...
  `--user-data-dir=${path.join(tmpdir(), `rogier-spotlight-map-${port}`)}`,
