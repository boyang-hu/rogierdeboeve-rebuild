...
const outDir = process.env.OUT_DIR || path.join(tmpdir(), "rogier-spotlight-transfer");
...
const chrome = spawn(chromePath, [
  ...
  `--user-data-dir=${path.join(tmpdir(), `rogier-spotlight-transfer-${port}`)}`,
], { stdio: ["ignore", "ignore", "pipe"] });

try {
  await waitForPort(port);
  const results = [];
  results.push(await captureVariant("rebuild-home-spotlight-transfer-default", ""));
  results.push(await captureVariant("rebuild-home-spotlight-transfer-srgb", "&debug-spotlight-map-transfer=srgb"));
