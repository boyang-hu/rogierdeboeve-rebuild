...
const outDir = process.env.OUT_DIR || path.join(tmpdir(), "rogier-thumb-colorspace");
...
async function captureVariant(name, query) {
  ...
  await send(ws, "Page.navigate", { url: `${rebuildUrl}/?skip-preloader&debug-thumb-probe=1${query}` });
  ...
  if (!parsed.probe) throw new Error(`No __rogierThumbProbe data for ${name}`);
...
const chrome = spawn(chromePath, [
  ...
  `--user-data-dir=${path.join(tmpdir(), `rogier-thumb-colorspace-${port}`)}`,
