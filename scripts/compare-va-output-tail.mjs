...
const outDir = process.env.OUT_DIR || path.join(tmpdir(), "rogier-va-output-tail");
const port = Number(process.env.CDP_PORT || 9274);
...
async function captureVariant(name, query) {
  const failures = [];
  const exceptions = [];
  ...
  await send(ws, "Page.navigate", { url: `${rebuildUrl}/?skip-preloader${query}` });
  ...
  return {
    ...
    exceptions,
    consoleMessages: consoleMessages.filter((message) => /Shader Error|WebGLProgram|exception|warning/i.test(message)),
  };
}
...
const chrome = spawn(chromePath, [
  ...
  `--user-data-dir=${path.join(tmpdir(), `rogier-va-output-tail-${port}`)}`,
], { stdio: ["ignore", "ignore", "pipe"] });

try {
  await waitForPort(port);
  const results = [];
  results.push(await captureVariant("rebuild-home-va-tail-default", ""));
  results.push(await captureVariant("rebuild-home-va-tail-source", "&debug-va-output-tail=source"));
